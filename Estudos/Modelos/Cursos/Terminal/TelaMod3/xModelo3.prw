...
Local nQtdVen := nPrcven := 0     
	...
	For nI := 1 To Len( aCOLS ) //aCOLS - quantidades de linhas no grid
		...
		If aCOLS[nI,Len(aHeader)+1] // Se a linha for deletada, não calcula
			Loop
		Endif
...
Local nX := 0
Local nI := 0
Local nProd := GdFieldPos("Z3_CODPRO") // Pega a posição no grid do campo especificado
	...
	// Grava os itens
		...
		For nX := 1 To Len( aCOLS )
			...
			If ! aCols[nX][Len(aHeader)+1] .AND. ! Empty(aCols[nX,nProd])      //! aCols[nX][Len(aHeader)+1] -  Valida se a linha esta deletada | .AND. ! Empty(aCols[nX,nProd]) - verifica se a linha e o produto estão vazio
				...
				RecLock( cAlias2, .T. )
					...
					SZ3->Z3_COD    := M->Z2_COD //Utilizar a variavel de memoria, pois o cabeçalho é gravado depois do item
				MsUnLock()                      //Caso o cabeçalho serja gravado primeiro, pode-se utilizar SZ2->Z2_COD,
			Endif                               //mas o mais recomendado é sempre utilziar a variavel de memória M->
		Next nX
