...
Private aTitles		:= {"Cad. Cliente","Fornecedor","Produto","Pedido"} //Nomes das abas
Private aCliente	:= {{"","","","","","//"}}
Private aFornece	:= {{"","","","",""}}
Private aProdutos	:= {{"","","",""}}
Private aPedidos    := {{"","","",""}}
...
oDlg := MsDialog():New(000,000,550,950,"Exemplo de LisBox",,,.F.,,,,,,.T.,,,.T.)  //Cria a tela

oFld := tFolder():New(025,005,aTitles,{},oDlg,,,,.T.,.F.,468,228) //aTitles - cria o nome das abas
...
//===================================================================== Folder 1 - Cliente ======================================================================
//Listbox - grid nao editavel | Header - cabeçalho                                                        Tamanho do objeto     Objeto que tras os dados                    
...
//SetArray - carrega os dados do objeto aCliente
...
//===================================================================== Folder 4 - aPedidos ======================================================================== 							
@ 005,005 LISTBOX oLbx4 FIELDS HEADER 	"Codigo ","Nome ", "Data ","Produto " SIZE 456,205 PIXEL OF oFld:aDialogs[4]
oLbx4:SetArray(aPedidos)
oLbx4:bLine := { || {	aPedidos[oLbx4:nAt,01],aPedidos[oLbx4:nAt,02],aPedidos[oLbx4:nAt,03],aPedidos[oLbx4:nAt,04] } } 							



//******************************************************************************************************
oBtn := tButton():New(258,435,"Sair",oDlg,{||Sair()},036,012,,,,.T.,,"",,,,.F.) //Botão sair
//oLbx2:blDbClick
...
dbSelectArea("SA1") //Abre a tabela
SA1->( dbSetOrder(1) ) //Seleciona o indice 1
SA1->( dbGoTop())
...
cSql +="       A2_EST,    "
cSql +="       A2_MSBLQL  "
cSql +=" FROM " + RetSQLName("SA2") //RetSQLName - verifica a empresa logada e tras a tabela referente a ela
cSql +="  Where A2_FILIAL = '" + xFilial("SA2") + "'"
cSql +="  AND D_E_L_E_T_ = ' '  "

cSql := ChangeQuery(cSql) //ChangeQuery - Trata caixa alta, retira espaço dos dados

dbUseArea( .T.,"TOPCONN", TCGENQRY(,,cSql),(cAliasFor), .F., .T.) //dbUseArea - cria uma area |TCGENQRY - cria uma tabela virtual com os dados
...
cSql :=" Select Z1_CODIGO, "
cSql +="	    Z1_NOME,   "
cSql +="        Z1_DATA,   "
cSql +="        Z1_PRODUTO "
cSql +=" FROM " + RetSQLName("SZ1") //RetSQLName - verifica a empresa logada e tras a tabela referente a ela
cSql +="  Where Z1_FILIAL = '" + xFilial("SZ1") + "'"
cSql +="  AND D_E_L_E_T_ = ' '  "

cSql := ChangeQuery(cSql) //ChangeQuery - Trata caixa alta, retira espaço dos dados

dbUseArea( .T.,"TOPCONN", TCGENQRY(,,cSql),(cAliasPed), .F., .T.) //dbUseArea - cria uma area |TCGENQRY - cria uma tabela virtual com os dados
...
While ! (cAliasPed)->( EOF() ) 
        ...
        AADD ( aPedidos, { (cAliasPed)->Z1_CODIGO ,;
 	  				       (cAliasPed)->Z1_NOME   ,;
 	  				       (cAliasPed)->Z1_DATA   ,;
 	  			      	   (cAliasPed)->Z1_PRODUTO } )
