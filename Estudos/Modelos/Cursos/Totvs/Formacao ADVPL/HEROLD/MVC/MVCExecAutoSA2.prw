...
Static Function Import( cAlias, aCampos, cFonte )
	...
	dbSelectarea(cAlias)
	dbSetOrder(1)
	msSeek(xFilial(cAlias) + aCampos[1,2]+aCampos[2,2] )
  ...
  // Desativando validaçao	   
	...
	// É feita a atribuição do dado ao campo do Model
			If !( lAux := oModel:SetValue( "SA2MASTER", aCampos[nI][1], aCampos[nI][2] ) )
