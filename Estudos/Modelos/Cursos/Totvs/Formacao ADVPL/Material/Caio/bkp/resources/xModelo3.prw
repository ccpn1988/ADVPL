...
// Atualização do total
 ...
 ACTIVATE MSDIALOG oDlg ON INIT EnchoiceBar(oDlg,{|| IIF( Obrigatorio( aGets, aTela ) .AND. U_Mod3TOk(),;
 														( nOpcA := 1, oDlg:End() ), NIL ) },{|| oDlg:End() })
...
If nOpc <> 3 // diferente de incluir
	...
	/*While ! (cAlias2)->( EOF() ) .AND. ( (Alias2)->Z3_FILIAL == xFilial(cAlias2) .AND. (cAlias2)->Z3_COD == cChave )*/
	While ! (cAlias2)->( EOF() ) .AND. ( (cAlias2)->Z3_FILIAL == xFilial(cAlias2) .AND. (cAlias2)->Z3_COD == cChave )	//Marcus Vinicius - 11/05/19 - ajustado o uso da variável Alias2 para cAlias2
		AADD( aREG, SZ3->( RecNo() ) )
...
Local nQtdVen	:= nPrcven := 0
	...
	nTotal	:= 0	//Marcus Vinicius - 11/05/19 - Zerar totalizador antes do calculo
...
Local nI := 0
		...
		For nX := 1 To Len( aCOLS )
			If ! aCols[nX][Len(aHeader)+1]       // Valida se a linha esta deletada
				...
				RecLock( cAlias2, .T. )
					...
					SZ3->Z3_COD    := M->Z2_COD			//Marcus Vinicius - 11/05/19
				MsUnLock()
			Endif
		Next nX
	...
	If nOpc == 5 // EXCLUSãO
		BEGIN TRANSACTION
			...
			dbSeek(xFilial(cAlias2) + SZ2->Z2_COD)
			...
			//DisarmTransaction() // utilizado para desfazer a transação dentro de uma estrutura de validação
