...
// Atualização do total
 ...
 ACTIVATE MSDIALOG oDlg ON INIT EnchoiceBar(oDlg,{|| IIF( Obrigatorio( aGets, aTela ).AND. u_Mod3OK() ,; 
...
Local nI := 0
	...
	If nOpc == 5 // EXCLUSãO
		...
		While ! (cAlias2)->(EOF()) .AND. ( (cAlias2)->Z3_FILIAL+(cAlias2)->(Z3_COD) == xFilial(cAlias1)+SZ2->Z2_COD)
			RecLock(cAlias2, .F.)
				dbDelete()
			MsUnLock()
			(cAlias2)->(dbSkip())
