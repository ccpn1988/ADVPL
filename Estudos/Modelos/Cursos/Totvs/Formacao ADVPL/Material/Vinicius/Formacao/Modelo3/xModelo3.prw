...
// Atualização do total
 ...
 ACTIVATE MSDIALOG oDlg ON INIT EnchoiceBar(oDlg,{|| IIF( Obrigatorio( aGets, aTela ) .AND. U_Mod3TOk(), ( nOpcA := 1, oDlg:End() ), NIL ) },{|| oDlg:End() })
...
Local nI := 0
	...
	If nOpc == 5 // EXCLUSãO
		/*USADO PARA GARANTIR A GRAVAÇÂO NO BANCO - USADO PARA GRAVÇÔES DE GRANDES VOLUMES*/
		BEGIN TRANSACTION
			// Deleta o Cabeçalho
			/*NÂO PRECISAMOS POSICIONAR UMA VEZ QUE O REGISTRO CLICACO JÀ ESTÀ POSICIONADO*/
			dbSelectArea(cAlias1)
			RecLock(cAlias1,.F.)
				dbDelete()
			MsUnLock()
			...
			dbSelectArea(cAlias2)
			dbSetOrder(1)
			/*SEMPRE DEVE SER CHAMADO A TABELA PAI DO CABEÇALHO ONDE TEM OS ITENS*/
			...
			While ! (cAlias2)->(EOF()) .AND. ( (cAlias2)->Z3_FILIAL+(cAlias2)->(Z3_COD) == xFilial(cAlias1)+M->Z2_COD)
				...
				(cAlias2)->(dbSkip())
			EndDo
			// DISARMTRANSACTION() //USADO DENTRO DE UMA VALIDAÇÂO PARA NÂO CONTINUAR COM A GRAVAÇÃO.
		END TRANSACTION
	Endif   
