...
Local oReport
	...
	//Chamando a Função para criar a estrutura do relatorio	
	oReport := ReportDef()
...
oReport := TReport():New("FornecedorXpedido","Relatorio de Fornecedor X Pedido",,{|oReport| PrintReport(oReport)},"Relatório Teste")
oReport:SetLandscape() 
oSection1 := TRSection():New(oReport,"Dados do Fornecedor",{"SZ2"},{"Código","Z2_COD"} )
...
// Quebra por Vendedor
	oBreak := TRBreak():New(oSection1,oSection1:Cell("Z2_COD"),"Sub Total Código")
...
oSection:BeginQuery() //método que imprime o resultado de uma Query
