...
User Function GENI018F()

Local _aArea := GetArea()
Private lDigital := .F.
Private lGeraNF := .F. //PARAMETRO PARA INDICAR SE DEVE GERAR A NOTA (.T.) OU SOMENTE O PEDIDO (.F.)
...
_cView	:= SUPERGETMV("MV_XVIEWPF",.T.,"DBA_EGK.TT_I29_PEDIDOS_FORUM") //Parâmetro que contém o nome da view que será consultada para a criação do Pedido de Vendas
...
_cLogPd	:= SUPERGETMV("MV_XLOGPF",.T.,"\logsiga\ped forum\") //Parâmetro que contém o caminho onde será gravado o arquivo de log de inconsistências
...
Local _aTmpPV1			:= {}
Local _aPVlNFs			:= {}
Local _cMvSeri 			:= Iif(lDigital,"2",GetMv("GEN_FAT003"))//GetMv("GEN_FAT003") //SERIE nota de saída
Local _cNotaImp			:= ""
Local _nPosLb			:= 0
...
Private _cView			:= SUPERGETMV("MV_XVIEWPF",.T.,"DBA_EGK.TT_I29_PEDIDOS_FORUM") //Parâmetro que contém o nome da view que será consultada para a criação do Pedido de Vendas
Private _cLogPd			:= SUPERGETMV("MV_XLOGPF",.T.,"\logsiga\ped forum\") //Parâmetro que contém o caminho onde será gravado o arquivo de log de inconsistências
