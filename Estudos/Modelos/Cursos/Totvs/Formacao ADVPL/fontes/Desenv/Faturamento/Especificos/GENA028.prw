...
If Len(cMGet1) > 0 .and. !Empty(cTpPub)
	...
	IF FunName()=="MATA410" .AND. Len(aOcorr) <> 0
		...
		MemoWrite("\spool\GENA084_"+M->C5_NUM+".log",cLogAux)
	...
	ElseIF FunName()=="MATA415" .AND. Len(aOcorr) <> 0
		...
		cLogAux := ""
		aEval(aOcorr, {|x| cLogAux+=x+chr(13)+chr(10) } )
		MemoWrite("\spool\GENA084_"+M->CJ_NUM+".log",cLogAux)
...
For nAtual := 1 to nTotReg
	...
	If !lB1_Seek
		If nQt < 2 .AND.lObraCanc == .T. //VAIDAÇÂO DE OBRA CANCELADA, ALTERANDO A DESCRIÇÃO JUNTO AO ESPELHO DO PEDIDO - 13/05/2019 - CAIO NEVES
			aAdd(aOcorr, "ISBN "+cProduto+" Obra Fora de catálogo!")
		ElseIf nQt < 2	
