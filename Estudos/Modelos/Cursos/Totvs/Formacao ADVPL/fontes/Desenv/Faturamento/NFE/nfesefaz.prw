...
//³Cleuto Lima - 30/03/2017                               ³
//³                                                       ³
//³Tratamento para buscar a natureza da operação apenas em³
//³na TES.                                                ³
//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
Local cTesNatOper	:= SuperGetMv("GEN_FAT150",.F.,"")

//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
//³Cleuto Lima - 11/09/2017                               ³
//³                                                       ³
//³Identifica se o cliente é exterior                     ³
//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
Local lCliEx		:= .F.
...
cMsgGen += AllTrim(GetMv("GEN_FAT060")) + " "
cMsgGen += AllTrim(GetMv("GEN_FAT061")) + " "
cMsgGen += AllTrim(GetMv("GEN_FAT062")) + " "
cMsgGen += AllTrim(GetMv("GEN_FAT063")) + " "
	...
	For nCountNF := 1 To Len(aCupRefLoj)
		...
		If MsSeek(xFilial("SF2")+cNota+cSerie+cClieFor+cLoja)
			...
			ElseIf FunName() == "SPEDNFSE"
				...
				//ALTERADO POR RODRIGO MOURAO - GEN - TRATAR O CAMPO XENDNUM E MONTAR O ENDERECO CORRETAMENTE
				aadd(aDest,FisGetEnd(SA1->A1_END,SA1->A1_EST)[1])
				if !Empty(SA1->A1_XENDNUM)
					_xNumGen := SA1->A1_XENDNUM
				elseif FisGetEnd(SA1->A1_END,SA1->A1_EST)[2]<>0
					_xNumGen := FisGetEnd(SA1->A1_END,SA1->A1_EST)[2]
				else
					_xNumGen :=  "SN"
				endif
				aadd(aDest,_xNumGen)
			...
			lCliEx	:= SF2->F2_EST == "EX"
				...
				If !SF2->F2_TIPO $ "DB" 
					...
					*/
					...
					If SF2->(FieldPos("F2_CLIENT"))<>0 .And. !Empty(SF2->F2_CLIENT+SF2->F2_LOJENT) .And. (SF2->F2_CLIENT+SF2->F2_LOJENT<>SF2->F2_CLIENTE+SF2->F2_LOJA .OR. cTpOper == "1" )
						...
						If MsSeek(xFilial("SA1")+SF2->F2_CLIENT+SF2->F2_LOJENT)
							...
							aadd(aEntrega,MyGetEnd(SA1->A1_END,SA1->A1_XENDNUM,"SA1")[1]) //ALTERADO POR DANILO AZEVEDO - GEN - PARA CONSIDERAR "NUMERO" EM CAMPO SEPARADO
							aadd(aEntrega,ConvType(IIF(MyGetEnd(SA1->A1_END,SA1->A1_XENDNUM,"SA1")[2]<>0,MyGetEnd(SA1->A1_END,SA1->A1_XENDNUM,"SA1")[2],"SN"))) //ALTERADO POR DANILO AZEVEDO - GEN - PARA CONSIDERAR "NUMERO" EM CAMPO SEPARADO
							aadd(aEntrega,MyGetEnd(SA1->A1_END,SA1->A1_XENDNUM,"SA1")[4]) //ALTERADO POR DANILO AZEVEDO - GEN - PARA CONSIDERAR "NUMERO" EM CAMPO SEPARADO
					...
					*/
					...
					//Mensagem personalizada GEN
					cMensCli += cMsgGen
					
					//Local de entrega GEN
					_aArea1 := SA1->(GetArea())
					_aArea5 := SC5->(GetArea())
					...
					Do Case
						Case SC5->(DbSeek(xFilial("SC5")+SD2->D2_PEDIDO)) .AND. !Empty(SC5->C5_XLOGENT)
	
							cMensCli += "Local de entrega: " + U_MyAltCarEsp( Alltrim(UPPER(Alltrim(NoAcento(SC5->C5_XNOMENT)))+' '+UPPER(Alltrim(NoAcento(SC5->C5_XSNOENT)))) )
							cMensCli += ", End:" + U_MyAltCarEsp( UPPER(NoAcento(Alltrim(SC5->C5_XLOGENT))) ) ;
							
							If !Empty(NoAcento(Alltrim(SC5->C5_XNUMENT)))
								cMensCli += ", " + U_MyAltCarEsp( UPPER(NoAcento(Alltrim(SC5->C5_XNUMENT))) )
							EndIf
							
							If !Empty(NoAcento(Alltrim(SC5->C5_XCOMENT)))
								cMensCli += ", " + U_MyAltCarEsp( UPPER(NoAcento(Alltrim(SC5->C5_XCOMENT))) )
							EndIf    
							
							If !Empty(NoAcento(Alltrim(SC5->C5_XCEPENT)))
								cMensCli += ",CEP: " + transform(UPPER(NoAcento(Alltrim(SC5->C5_XCEPENT))), "@R 99999-999")
							EndIf
							
							If !Empty(NoAcento(Alltrim(SC5->C5_XBAIENT)))
								cMensCli += ", " + U_MyAltCarEsp( UPPER(NoAcento(Alltrim(SC5->C5_XBAIENT))) )
							EndIf
							
							If !Empty(NoAcento(Alltrim(SC5->C5_XUFENT)))
								cMensCli += ", " + UPPER(NoAcento(Alltrim(SC5->C5_XUFENT)))
							EndIf	
	
							If !Empty(NoAcento(Alltrim(SC5->C5_XPTREF)))
								cMensCli += ", " + UPPER(NoAcento(Alltrim(SC5->C5_XPTREF)))
							EndIf
						...
						OtherWise
							...
							If !Empty(SF2->F2_CLIENT)
								SA1->(DbSetOrder(1))
								If SA1->(DbSeek(xFilial("SA1")+SF2->F2_CLIENT+SF2->F2_LOJENT))
									cMensCli += "Local de entrega: "	+ U_MyAltCarEsp( Alltrim(SA1->A1_END) ) ;
									+ ", " + U_MyAltCarEsp( Alltrim(SA1->A1_XENDNUM) ) ;
									+ ", " + U_MyAltCarEsp( Alltrim(SA1->A1_COMPLEM) ) ;
									+ ", " + U_MyAltCarEsp( Alltrim(SA1->A1_BAIRRO) ) ;
									+ ", " + U_MyAltCarEsp( Alltrim(SA1->A1_EST) ) + "."
								Endif
							Endif
						
					EndCase
									
					//14/07/15 - Danilo - adicionado campo
					cXPedCli := Posicione("SC5",1,xFilial("SC5")+SD2->D2_PEDIDO,"C5_XPEDCLI")
					If !Empty(cXPedCli) //NUMERO DO PEDIDO DO CLIENTE - GEN
						cMensCli += " ## REF. PEDIDO "+alltrim(cXPedCli)+" ## "
					Endif
					...
					aadd(aDest,AllTrim(SA1->A1_CGC))
					aadd(aDest,SA1->A1_NOME)
				aadd(aDest,MyGetEnd(SA1->A1_END,SA1->A1_XENDNUM,"SA1")[1]) //ALTERADO POR DANILO AZEVEDO - GEN - PARA CONSIDERAR "NUMERO" EM CAMPO SEPARADO
					
				If MyGetEnd(SA1->A1_END,SA1->A1_XENDNUM,"SA1")[2]<>0 //ALTERADO POR DANILO AZEVEDO - GEN - PARA CONSIDERAR "NUMERO" EM CAMPO SEPARADO
					aadd(aDest,MyGetEnd(SA1->A1_END,SA1->A1_XENDNUM,"SA1")[3]  )  //ALTERADO POR DANILO AZEVEDO - GEN - PARA CONSIDERAR "NUMERO" EM CAMPO SEPARADO					
					Else
						aadd(aDest,"SN")
					EndIf
	
				aadd(aDest,IIF(SA1->(FieldPos("A1_COMPLEM")) > 0 .And. !Empty(SA1->A1_COMPLEM),SA1->A1_COMPLEM,MyGetEnd(SA1->A1_END,SA1->A1_XENDNUM,"SA1")[4])) //ALTERADO POR DANILO AZEVEDO - GEN - PARA CONSIDERAR "NUMERO" EM CAMPO SEPARADO
					aadd(aDest,SA1->A1_BAIRRO)
					If !Upper(SA1->A1_EST) == "EX"
						aadd(aDest,SA1->A1_COD_MUN)
	          ...
	          // Posiciono na TES do cupom fiscal para pegar a natureza de operação da nf sobre cupom
						...
						if FindFunction( "SpedNatOper" ) .And. SFT->( MsSeek( xFilial( "SFT" ) +cChave) )
							If SF4->F4_CODIGO $ cTesNatOper	.AND. !Empty(Alltrim(SF4->F4_TEXTO))
								If Empty(cNatOper)
									cNatOper := Alltrim(SF4->F4_FINALID)
								Else
									cNatOper += Iif(!Alltrim(SF4->F4_FINALID)$cNatOper,"/ " + SF4->F4_FINALID,"")
								Endif
							Else
		...
		//						If MsSeek(xFilial("SD2")+(cAliasSD2)->D2_NFORI+(cAliasSD2)->D2_SERIORI+(cAliasSD2)->D2_CLIENTE+(cAliasSD2)->D2_LOJA+(cAliasSD2)->D2_COD+(cAliasSD2)->D2_ITEMORI)
						...
						If !AllTrim(SC5->C5_MENNOTA) $ cMensCli
							...
							EndIf                       
						If !Empty(cFieldMsg) .and. SC5->(FieldPos(cFieldMsg)) > 0 .and. !Empty(&("SC5->"+cFieldMsg))
							cMensCli += alltrim(&("SC5->"+cFieldMsg)) //ALTERADO POR DANILO AZEVEDO - GEN
						Else
						...
						If !Empty(SC6->C6_NUMPCOM) .OR. !Empty(SC5->C5_XPEDCLI) //.And. !Empty(SC6->C6_ITEMPC)
							If !Empty(SC5->C5_XPEDCLI)
								aadd(aPedCom,{SC5->C5_XPEDCLI,""})
							Else	
			            ...
			            If cMvMsgTrib $ "1-3"
				            ...
				            EndIf
			            EndIf
					_cIsbn := Posicione("SB1",1,xFilial("SB1")+cCodProd,"B1_ISBN") //ADICIONADO POR DANILO AZEVEDO - GEN
					...
					Iif(Empty(_cIsbn),cCodProd,_cIsbn),; //cCodProd,; //ALTERADO POR DANILO AZEVEDO - GEN
							...
							IIF(Empty(SB5->B5_UMDIPI) .OR. !lCliEx ,SB1->B1_UM,SB5->B5_UMDIPI),;
							IIF(Empty(SB5->B5_CONVDIP) .OR. !lCliEx ,(cAliasSD2)->D2_QUANT,SB5->B5_CONVDIP*(cAliasSD2)->D2_QUANT),;
							(cAliasSD2)->D2_VALFRE,;
							(cAliasSD2)->D2_SEGURO,;
	...
	If MsSeek(xFilial("SF1")+cNota+cSerie+cClieFor+cLoja)
		...
		Else				
			...
			If SF1->F1_TIPO $ "DB" 
				...
				EndIf
				// cleuto - 18/06/2018 tratamento comento pois será reavaliado posteriormente
				If SF1->(FieldPos("F1_FORRET"))<>0 .And. !Empty(SF1->F1_FORRET+SF1->F1_LOJARET) .And. SF1->F1_FORRET+SF1->F1_LOJARET <> SF1->F1_FORNECE+SF1->F1_LOJA
					...
					IF MsSeek(xFilial("SA1")+SF1->F1_FORRET+SF1->F1_LOJARET)
						...
						aadd(aRetirada,SA1->A1_CGC)
						aadd(aRetirada,MyGetEnd(SA1->A1_END,SA1->A1_XENDNUM,"SA1")[1])
						aadd(aRetirada,ConvType(IIF(MyGetEnd(SA1->A1_END,SA1->A1_XENDNUM,"SA1")[2]<>0,MyGetEnd(SA1->A1_END,SA1->A1_XENDNUM,"SA1")[2],"SN")))
						aadd(aRetirada,MyGetEnd(SA1->A1_END,SA1->A1_XENDNUM,"SA1")[4])
						aadd(aRetirada,SA1->A1_BAIRRO)
						aadd(aRetirada,SA1->A1_COD_MUN)
						aadd(aRetirada,SA1->A1_MUN)
						aadd(aRetirada,Upper(SA1->A1_EST))
				...
				If SF1->(FieldPos("F1_FORENT")) <> 0 .And. !Empty(SF1->F1_FORENT+SF1->F1_LOJAENT) .And. SF1->F1_FORENT+SF1->F1_LOJAENT <> SF1->F1_FORNECE+SF1->F1_LOJA
					...
					If MsSeek(xFilial("SA1")+SF1->F1_FORENT+SF1->F1_LOJAENT)
						...
						aadd(aEntrega,SA1->A1_CGC)
						aadd(aEntrega,MyGetEnd(SA1->A1_END,SA1->A1_XENDNUM,"SA1")[1])
						aadd(aEntrega,ConvType(IIF(MyGetEnd(SA1->A1_END,SA1->A1_XENDNUM,"SA1")[2]<>0,MyGetEnd(SA1->A1_END,SA1->A1_XENDNUM,"SA1")[2],"SN")))
						aadd(aEntrega,MyGetEnd(SA1->A1_END,SA1->A1_XENDNUM,"SA1")[4])
						aadd(aEntrega,SA1->A1_BAIRRO)
						aadd(aEntrega,SA1->A1_COD_MUN)
						aadd(aEntrega,SA1->A1_MUN)
						aadd(aEntrega,Upper(SA1->A1_EST))
			...
			If SF1->F1_TIPO $ "DB" 
				...
				dbSetOrder(1)
				MsSeek(xFilial("SA1")+SF1->F1_FORNECE+SF1->F1_LOJA)	
				lCliEx	:= SF1->F1_EST == 'EX'
			Else
			    dbSelectArea("SA2")
				dbSetOrder(1)
				MsSeek(xFilial("SA2")+SF1->F1_FORNECE+SF1->F1_LOJA)	
				/* vivaz 39741 - para notas de importação devemos usar sempre UN */
				lCliEx	:= .F.//SF1->F1_EST == 'EX'
			EndIf
			...
			While !Eof() .And. xFilial("SD1") == (cAliasSD1)->D1_FILIAL .And.;
				...
				_cIsbn := Posicione("SB1",1,xFilial("SB1")+(cAliasSD1)->D1_COD,"B1_ISBN") //ADICIONADO POR DANILO AZEVEDO - GEN
				//Tratamento para que o valor de ICMS ST venha a compor o valor da tag vOutros quando for uma nota de Devolução, impedindo que seja gerada a rejeição 610.
		       nIcmsST := 0
		       ...
		       EndIf   		
				...
				aadd(aProd,	{Len(aProd)+1,;  
				Iif(Empty(_cIsbn),cCodProd,_cIsbn),; //(cAliasSD1)->D1_COD,; //ALTERADO POR DANILO AZEVEDO - GEN
					IIf(Val(SB1->B1_CODBAR)==0,"",StrZero(Val(SB1->B1_CODBAR),Len(Alltrim(SB1->B1_CODBAR)),0)),;
					cDescProd,;
					SB1->B1_POSIPI,;
					SB1->B1_EX_NCM,;
				...
				IIF(!(cAliasSD1)->D1_TIPO$"IP",IIF(!(SubStr((cAliasSD1)->D1_CF,1,1) $ "3"),(cAliasSD1)->D1_TOTAL,(cAliasSD1)->D1_TOTAL - (cAliasSD1)->D1_II),0),;
					IIF(Empty(SB5->B5_UMDIPI) .OR. !lCliEx ,SB1->B1_UM,SB5->B5_UMDIPI),;
					IIF(Empty(SB5->B5_CONVDIP) .OR. !lCliEx ,(cAliasSD1)->D1_QUANT,SB5->B5_CONVDIP*(cAliasSD1)->D1_QUANT),;
					(cAliasSD1)->D1_VALFRE,;
					(cAliasSD1)->D1_SEGURO,;
...
IF lPe01Nfe     
	...
	aParam := {aProd,cMensCli,cMensFis,aDest,aNota,aInfoItem,aDupl,aTransp,aEntrega,aRetirada,aVeiculo,aReboque,aNfVincRur,aEspVol,aNfVinc,aDetPag,aObsCont,cTipo}
...
o comando a sSort foi removido devido a NT 2019.001
MP-NFE- 927 -Rejeição: Número do item fora da ordem sequencial

//aSort(aProd,,, { | x ,y| x[4] < y[4] } ) //GEN - ORDENAR OS ITENS PELA DESCRICAO
*/
...
If !Empty(aNota)
	...
	IF cVeramb >= "4.00"
       ...
       //If aScan( aDetPag,{ |x|x[1] == "14"} ) > 0			
		If lGrupCob
			cString += NfeCob(aDupl,aFat,(Alltrim(cSerie)+ Alltrim(cNota)),aDetPag)
		EndIf
		//EndIf
...
Static Function NfeDest(aDest,cVerAmb,aTransp,aCST,lBrinde,cMunDest)
	...
	EndIf
		...
		cString += '<Nome>'+U_MyAltCarEsp(ConvType(iIf(!lBrinde,aDest[02],"Diversos - Brindes")), .T.)+'</Nome>'
		cString += '<enderDest>'
		cString += '<Lgr>'+U_MyAltCarEsp(ConvType(iIf(!lBrinde,aDest[03],(FisGetEnd(SM0->M0_ENDENT,SM0->M0_ESTENT)[1]))))+'</Lgr>'	
		if lBrinde
		...
		cString += '<nro>'+U_MyAltCarEsp(Alltrim(Str(aDest[04])))+'</nro>'
		Else
		cString += '<nro>'+U_MyAltCarEsp(ConvType(aDest[04]))+'</nro>'
		EndIf
	endif
cString += NfeTag('<Cpl>',U_MyAltCarEsp(ConvType(iIf(!lBrinde,aDest[05],SM0->M0_COMPENT))))
	cString += '<Bairro>'+U_MyAltCarEsp(ConvType(iIf(!lBrinde,aDest[06],SM0->M0_BAIRENT)))+'</Bairro>'
	cString += '<cMun>'+ConvType(iIf(!lBrinde,aUF[aScan(aUF,{|x| x[1] == aDest[09]})][02]+aDest[07],SM0->M0_CODMUN))+'</cMun>'
	cString += '<Mun>'+U_MyAltCarEsp(ConvType(iIf(!lBrinde,aDest[08],SM0->M0_CIDENT)))+'</Mun>'
...
// Cleuto - 25/06/2018 - alterado para sempre retornar o EAN
//cEantrib	:= IIF(!Empty(aProd[45]),aProd[45], iif( aProd[08] <> aProd[11],"",cEan))
cEantrib	:= IIF(!Empty(aProd[45]),aProd[45], cEan)

// Cleuto - 18/07/2018 - alterado para quando ISBN não tiver o padrão de 13 digitos informar branco, para atender a rejeição de GTIN invalido para revistas
If Len(AllTrim(cEan)) <> 13
	cEan		:= ""
	cEantrib	:= ""
EndIf
...
If Len(aTransp)>0
	cString += '<transporta>'
		...
		EndIf
		cString += NfeTag('<Nome>' ,U_MyAltCarEsp(ConvType(aTransp[02])), .T.)
		cString += NfeTag('<IE>'    ,aTransp[03])
		cString += NfeTag('<Ender>',U_MyAltCarEsp(ConvType(aTransp[04])))
		cString += NfeTag('<Mun>'  ,U_MyAltCarEsp(ConvType(aTransp[05])))
...
Static Function NfeCob(aDupl,aFat,cFatura,aDetPag)
...
Local lAvista	:= .F.//<-- 43307 - Cleuto - 17/07/2018 - tratamento para não gerar a tag de parcelas quando pagamento a vista

DEFAULT cFatura	:= ""
DEFAULT aDupl	:= {}  
DEFAULT aFat	:= {}

//<-- 43307 - Cleuto - 17/07/2018 - tratamento para não gerar a tag de parcelas quando pagamento a vista
IF len(aDetPag) > 0  
...
If Len(aDupl)>0 
	...
	//IF !lAvista //<-- 43307 - Cleuto - 17/07/2018 - tratamento para não gerar a tag de parcelas quando pagamento a vista
		cString += '<cobr>'
...
Static Function MyGetEnd(cEndereco,cNumero,cAlias)
...
If Empty(cNumero) .OR. AllTrim(cNumero) == "0"
	cEndAux	:= AllTrim(cEndereco)
Else
	If AllTrim(cNumero) $ AllTrim(cEndereco)
		cEndAux	:= AllTrim(cEndereco)
	Else
		cEndAux	:= AllTrim(cEndereco)+" "+AllTrim(cNumero)
	EndIf	
EndIF
...
If (&(cAlias+"->"+cCmpEst) == "DF") .Or. ((cAlias)->(FieldPos(cCmpEndN)) > 0 .And. &(cAlias+"->"+cCmpEndN) == "1")
	...
	aRet[3] := "SN"
Else
	aRet := FisGetEnd(cEndAux, (&(cAlias+"->"+cCmpEst)))
EndIf
...
Local cChave	:= ""  
Local lAtuSC5	:= .F. 
Local cMsgNfs  := xRemoChar(SC5->C5_MENNOTA)
   ...
   SC5->( dbSetOrder(1) ) 
	...
	If lAtuSC5 .And. SC5->( MsSeek( xFilial("SC5") + cNumPed )   ) 
		If AllTrim(SC5->C5_MENNOTA) <> AllTrim(cMsgNfs)
			RecLock( "SC5" , .F. )
			SC5->C5_MENNOTA := xRemoChar(cMsgNfs)
			MsUnLock()	
		EndIf
	EndIf
...
/* 
	...
	FUNÇÕES CUSTOMIZADAS GRUPO GEN 

	TODAS AS FUNÇÕES ESPECIFICAS GRUPO GEN DEVEM SER CRIADAS A PARTIR DESTE PONTO
...
±±ºPrograma  ³NFESEFAZ  ºAutor  ³Microsiga           º Data ³  08/17/17   º±±
...
Static Function xRemoChar(cParamChar)

Local cRetXml	:= ""
Local aCharExp	:= {}
Local nAuxChar	:= 0
Local cRetXml	:= cParamChar
Local aCharExp	:= StrTokArr(SuperGetMv("GEN_FAT172",.f.,"<#>#&"),"#")

For nAuxChar := 1 To Len(aCharExp)

	cRetXml	:= StrTran(cRetXml, aCharExp[nAuxChar] , " " )
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±ÉÍÍÍÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍ»±±
±±ºPrograma  ³MyAltCarEsº Autor ³ Renato Calabro'    º Data ³  07/06/16   º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºDesc.     ³ Altera caracteres especiais do texto, caso encontre, para  º±±
±±º          ³ nao causar erro no schema do XML do Sefaz                  º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºUso       ³ Especifico                                                 º±±
±±ÈÍÍÍÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍ¼±±
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
...
User Function MyAltCarEsp(cTexto, lVirgula)
...
cRet := cTexto
cRet := Replace(cRet,'-','')
cRet := Replace(cRet,'Å','&Aring;')
cRet := Replace(cRet,'å','&aring;')
cRet := Replace(cRet,'Ä','&Auml;')
cRet := Replace(cRet,'ä','&auml;')
cRet := Replace(cRet,'Æ','&AElig;')
cRet := Replace(cRet,'æ','&aelig;')
cRet := Replace(cRet,'Ë','&Euml;')
cRet := Replace(cRet,'ë','&euml;')
cRet := Replace(cRet,'Ğ','&ETH;')
cRet := Replace(cRet,'ğ','&eth;')
cRet := Replace(cRet,'Ï','&Iuml;')
cRet := Replace(cRet,'ï','&iuml;')
cRet := Replace(cRet,'Ø','&Oslash;')
cRet := Replace(cRet,'ø','&oslash;')
cRet := Replace(cRet,'Ö','&Ouml;')
cRet := Replace(cRet,'ö','&ouml;')
cRet := Replace(cRet,'Ü','&Uuml;')
cRet := Replace(cRet,'ü','&uuml;')
cRet := Replace(cRet,'<','&lt;')
cRet := Replace(cRet,'>','&gt;')
cRet := Replace(cRet,'&','&amp;')
cRet := Replace(cRet,'"','&quot;')
cRet := Replace(cRet,'®','&reg;')
cRet := Replace(cRet,'©','&copy;')
cRet := Replace(cRet,'İ','&Yacute;')
cRet := Replace(cRet,'ı','&yacute;')
cRet := Replace(cRet,'Ş','&THORN;')
cRet := Replace(cRet,'ş','&thorn;')
cRet := Replace(cRet,'ß','&szlig;')   
cRet := Replace(cRet,"'",'&#39;')

//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
//³ Remove caracteres especiais de espaco                             ³
...
//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
//³ Tratamento para remover a virgula do texto enviado                ³
//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
If lVirgula .AND. At(",", cRet) > 0
	cRet := StrTran(cRet, ",", "")
EndIf
