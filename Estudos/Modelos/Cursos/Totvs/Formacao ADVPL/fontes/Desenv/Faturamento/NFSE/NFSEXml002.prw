...
If cTipo == "1"
	...
	If DbSeek(xFilial("SF2")+cNota+cSerie+cClieFor+cLoja)	
		...
		While !(cAliasSD2)->(Eof()) .And. xFilial("SD2") == (cAliasSD2)->D2_FILIAL .And.;
			...
			aSX560	:= FWGetSX5("60",RetFldProd(SB1->B1_COD,"B1_CODISS"))
			If Len(aSX560) > 0
				If !lNFeDesc  .And. nCont == 1
					If  cCodmun == "3300704-3156700"
						cNatOper := If(FindFunction('CleanSpecChar'),CleanSpecChar(AllTrim(SubStr(aSX560[1][4],1,55))),AllTrim(SubStr(aSX560[1][4],1,55))) + cNatOper
	    			Else
						cNatOper += If(FindFunction('CleanSpecChar'),CleanSpecChar(AllTrim(SubStr(aSX560[1][4],1,55))),AllTrim(SubStr(aSX560[1][4],1,55)))    			
	    			EndIf
	    	    ElseIf nCont == 1 
	    	        If cCodmun == "3300704-3156700"
						cDescrNFSe := If(FindFunction('CleanSpecChar'),CleanSpecChar(AllTrim(SubStr(aSX560[1][4],1,55))),AllTrim(SubStr(aSX560[1][4],1,55))) + cNatOper
	    			Else
						cDescrNFSe := If(FindFunction('CleanSpecChar'),CleanSpecChar(AllTrim(SubStr(aSX560[1][4],1,55))),AllTrim(SubStr(aSX560[1][4],1,55)))    			
	    			EndIf
	    	    EndIf 
    		EndIf
			//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
...
If cCodMun == "3550308" // Para o município de SP a tag deve ser gerada com 8 zeros nos casos abaixo
	cString += '<InscricaoMunicipalTomador>'+Iif('ISENTO' $ Upper(aDest[17]) .OR. Empty(aDest[17]),"00000000",aDest[17])+'</InscricaoMunicipalTomador>'
ElseIf cCodMun == "4113700"  //Londrina
	cString += '<InscricaoMunicipalTomador>'+Iif('ISENTO' $ Upper(aDest[17]) .OR. Empty(aDest[17]),"",aDest[17])+'</InscricaoMunicipalTomador>'
Else
	cString += '<InscricaoMunicipalTomador>'+Iif('ISENTO' $ Upper(aDest[17]) .OR. Empty(aDest[17]),"0000000",aDest[17])+'</InscricaoMunicipalTomador>'
EndIf		
