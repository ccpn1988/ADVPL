		...
		If DbSeek(xFilial("SF2")+cNota+cSerie+cClieFor+cLoja)	
			...
			While !(cAliasSD2)->(Eof()) .And. xFilial("SD2") == (cAliasSD2)->D2_FILIAL .And.;
				...
				//Pega descricao do pedido de venda-Parametro MV_NFESERV
           		cFieldMsg := GetNewPar("GEN_MSGNFS","")
				If !lNFeDesc
				...
				If !lAglutina .Or. Len(aProd) == 0
					...
					aadd(aProd,	{Len(aProd)+1,;
								...
								SF3->F3_ISSSUB,;
								SF3->F3_ISSMAT,;   
								xGenRetTrib()/*IIF(SB1->(FieldPos("B1_TRIBMUN"))<>0,RetFldProd(SB1->B1_COD,"B1_TRIBMUN"),"")*/,;
								SF3->F3_CODISS,;
								...
								xGenRetTrib()/*RetFldProd(SB1->B1_COD,"B1_TRIBMUN")*/,; //32
...
Static Function xGenRetTrib()

Local cRet	:= ""

If SF3->F3_FILIAL == "1001" .AND. AllTrim(SF3->F3_TRIBMUN) == "010901"
	cRet	:= SF3->F3_TRIBMUN
Else
	cRet := RetFldProd(SB1->B1_COD,"B1_TRIBMUN")
EndIF
