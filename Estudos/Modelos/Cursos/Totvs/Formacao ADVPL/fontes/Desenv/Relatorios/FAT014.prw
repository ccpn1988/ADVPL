...
ฑฑบPrograma  ณFAT014   บAutor  ณCleuto Lima - Loop  บ Data ณ  01/12/15   บฑฑ
ฑฑฬออออออออออุออออออออออสอออออออฯออออออออออออออออออออสออออออฯอออออออออออออนฑฑ
ฑฑบDesc.     ณRelatorio de vendas portal.                                 บฑฑ
ฑฑบ          ณ                                                            บฑฑ
...
oReport := TReport():New("FAT014","UTM_COD E-com"	,cPerg		,{|oReport| PrintReport(oReport)},"UTM_COD E-com") 
...
_cQuery := " SELECT DISTINCT PG.TIPO_PAGAMENTO PAGAMENTO, CODIGO_CUPOM,NUMERO NUM,C5_NUM,PED.COBRANCA_CPF_CNPJ CNPJ,PED.COBRANCA_NOME_RAZAO RAZAO,PED.COBRANCA_SOBRENOME_IE SOBRENOME,PED.COBRANCA_EMAIL EMAIL,PED.COBRANCA_UF UF,PED.COBRANCA_CIDADE CIDADE,'('||PED.COBRANCA_TELEFONE_DDD||')'||TO_CHAR(PED.COBRANCA_TELEFONE) TELEFONE,"+cQuebra
_cQuery += " '('||PED.COBRANCA_CELULAR_DDD||')'||TO_CHAR(PED.COBRANCA_CELULAR) CELULAR,TO_CHAR(DATA_PEDIDO,'YYYYMMDD') DT_PEDIDO,UTM_SOURCE,UTM_MEDIUM,PED.UTM_CAMPAIGN CAMPANHA,SX5TP.X5_DESCRI TIPOCLI"+cQuebra
_cQuery += " ,Z7_DESC AREA,Z5_DESC CURSO,SZ6.Z6_DESC DISCIPLINA,SX5Z2.X5_DESCRI,SKU,B1_ISBN,NOME,B1_DESC,B5_XCODHIS,B5_XDTPUBL,QUANTIDADE,PRECO,METODO_FRETE FRETE,VALOR_FRETE VALFRETE,ITEM.VALOR_DESCONTO VALDESC,SUBTOTAL,DATA_PEDIDO,TOTAL,(VALOR_FRETE/PED.QTY_ITENS)*QUANTIDADE FRETE_ITEM,GC.DESCRICAO GRUPOCLIENTE,"+cQuebra
_cQuery += " NVL(( "+cQuebra
_cQuery += "   SELECT SUM(QUANTIDADE) FROM genesb.pedido PEDB"+cQuebra
...
_cQuery += "   AND ITEMB.SKU = ITEM.SKU "+cQuebra
_cQuery += "   ),0) QTD_COMPRA, "+cQuebra
_cQuery += " TO_CHAR(( "+cQuebra
_cQuery += "   SELECT MAX(PEDB.DATA_PEDIDO) FROM genesb.pedido PEDB"+cQuebra
_cQuery += "   JOIN genesb.item ITEMB"+cQuebra
_cQuery += "   ON ITEMB.PEDIDO_ENTITY_ID = PEDB.ENTITY_ID"+cQuebra
_cQuery += "   WHERE PEDB.COBRANCA_CPF_CNPJ = PED.COBRANCA_CPF_CNPJ"+cQuebra
_cQuery += "   AND PEDB.ENTITY_ID < PED.ENTITY_ID"+cQuebra
_cQuery += "   AND ITEMB.SKU = ITEM.SKU "+cQuebra
_cQuery += "   ),'DD/MM/YYYY') DTULCOMP"+cQuebra
...
_cQuery += " INNER JOIN GENESB.PAGAMENTO PG"+cQuebra
_cQuery += " ON PED.ENTITY_ID = PG.PEDIDO_ENTITY_ID"+cQuebra  //Vivaz 30538 Beatriz Pinto
_cQuery += " INNER JOIN "+RetSqlName("SC5")+" SC5"+cQuebra
_cQuery += " ON TO_NUMBER(TRIM(C5_XPEDWEB)) = TO_NUMBER(TRIM(PED.NUMERO))"+cQuebra
_cQuery += " AND C5_EMISSAO >= TO_CHAR(PED.DATA_PEDIDO,'YYYYMMDD')"+cQuebra
_cQuery += " AND SC5.D_E_L_E_T_ <> '*'"+cQuebra
...
_cQuery += " JOIN "+RetSqlName("SC6")+" SC6"+cQuebra
_cQuery += " ON C5_FILIAL = C6_FILIAL"+cQuebra
_cQuery += " AND C5_NUM = C6_NUM"+cQuebra    
_cQuery += " AND C5_CLIENTE = C6_CLI"+cQuebra
_cQuery += " AND C5_LOJACLI = C6_LOJA"+cQuebra   
_cQuery += " AND C6_PRODUTO = B1_COD"+cQuebra   
_cQuery += " AND C6_TES IN(SELECT F4_CODIGO FROM " + RetSqlName("SF4") + " WHERE F4_DUPLIC = 'S' AND F4_TIPO = 'S' AND D_E_L_E_T_ = ' ')"+cQuebra      
_cQuery += " AND SC6.D_E_L_E_T_ <> '*'"+cQuebra

_cQuery += "INNER JOIN SA1000 SA1"+cQuebra
_cQuery += "ON SA1.A1_COD       = SC5.C5_CLIENTE"+cQuebra
_cQuery += "AND SA1.A1_LOJA     = SC5.C5_LOJACLI"+cQuebra
...
_cQuery += " SELECT DISTINCT PG.TIPO_PAGAMENTO PAGAMENTO, CODIGO_CUPOM,NUMERO NUM,C5_NUM,PED.COBRANCA_CPF_CNPJ CNPJ,PED.COBRANCA_NOME_RAZAO RAZAO,CASE WHEN LENGTH(PED.COBRANCA_CPF_CNPJ) <= 14 then PED.COBRANCA_SOBRENOME_FANTAZIA else PED.COBRANCA_IE END SOBRENOME,PED.COBRANCA_EMAIL EMAIL,PED.COBRANCA_UF UF,PED.COBRANCA_CIDADE CIDADE,TO_CHAR(PED.COBRANCA_TELEFONE) TELEFONE,"+cQuebra
_cQuery += " '0' CELULAR, TO_CHAR(DATA_PEDIDO,'YYYYMMDD') DT_PEDIDO,UTM_SOURCE,UTM_MEDIUM,PED.UTM_CAMPAIGN CAMPANHA,SX5TP.X5_DESCRI TIPOCLI"+cQuebra
_cQuery += " ,Z7_DESC AREA,Z5_DESC CURSO,SZ6.Z6_DESC DISCIPLINA,SX5Z2.X5_DESCRI,SKU,B1_ISBN,NOME,B1_DESC,B5_XCODHIS,B5_XDTPUBL,QUANTIDADE,PRECO,METODO_FRETE FRETE,VALOR_FRETE VALFRETE,ITEM.VALOR_DESCONTO VALDESC,SUBTOTAL,DATA_PEDIDO,TOTAL,(VALOR_FRETE/PED.QTY_ITENS)*QUANTIDADE FRETE_ITEM,GC.DESCRICAO GRUPOCLIENTE,"+cQuebra
_cQuery += " NVL(( "+cQuebra
_cQuery += "   SELECT SUM(QUANTIDADE) FROM genesb.pedido_novo PEDB"+cQuebra
...
_cQuery += "   ),0) QTD_COMPRA, "+cQuebra
_cQuery += " TO_CHAR(( "+cQuebra
_cQuery += "   SELECT MAX(PEDB.DATA_PEDIDO) FROM genesb.pedido_novo PEDB"+cQuebra
_cQuery += "   JOIN genesb.item_novo ITEMB"+cQuebra
_cQuery += "   ON ITEMB.PEDIDO_ENTITY_ID = PEDB.ENTITY_ID"+cQuebra
_cQuery += "   WHERE PEDB.COBRANCA_CPF_CNPJ = PED.COBRANCA_CPF_CNPJ"+cQuebra
_cQuery += "   AND PEDB.ENTITY_ID < PED.ENTITY_ID"+cQuebra
...
_cQuery += " AND TO_NUMBER(TRIM(B1_COD)) = TO_NUMBER(TRIM(ITEM.SKU))"+cQuebra
_cQuery += " AND SB1.D_E_L_E_T_ <> '*'"+cQuebra 
_cQuery += " INNER JOIN GENESB.PAGAMENTO_NOVO PG"+cQuebra
_cQuery += " ON PED.ENTITY_ID = PG.PEDIDO_ENTITY_ID"+cQuebra  //Vivaz 30538 Beatriz Pinto
_cQuery += " INNER JOIN "+RetSqlName("SC5")+" SC5"+cQuebra
_cQuery += " ON TO_NUMBER(TRIM(C5_XPEDWEB)) = TO_NUMBER(TRIM(PED.NUMERO))"+cQuebra
_cQuery += " AND C5_EMISSAO >= TO_CHAR(PED.DATA_PEDIDO,'YYYYMMDD')"+cQuebra
...
While !(_cAlias1)->(Eof())
	...
	oSection1:Cell("CPF"):SetValue(		    (_cAlias1)->CNPJ		)     
	...
	oSection1:Cell("CANAL"):SetValue(    	(_cAlias1)->TIPOCLI	)
	...
	oSection1:Cell("CODHIS"):SetValue(		(_cAlias1)->B5_XCODHIS )  
	oSection1:Cell("DATAPUB"):SetValue(		(_cAlias1)->B5_XDTPUBL )
	oSection1:Cell("GRUPOCLIENTE"):SetValue(		(_cAlias1)->GRUPOCLIENTE )
