...
ฑฑบ          ณ                                                            บฑฑ
ฑฑฬออออออออออุออออออออออหออออออออออออออออออออออออออออหออออออัอออออออออออออนฑฑ
ฑฑบAltera็ใo ณAutor     บJoni Fujiyama               บ Data ณ  02/07/14   บฑฑ
...
While !LockByName("GENI017A",.T.,.T.,.T.)
	...
	Sleep(10)
	If nX > 2     
		Conout("GENI017A - Nใo foi possํvel executar a rotina importa็ใo de Produtos neste momento pois a fun็ใo GENI017A jแ esta sendo executada por outra processamento!"+DTOC(DDataBase)+" "+Time())
		Return .F.
    EndIf
EndDo
...
If Val(aTime[1]) > 0 .OR. Val(aTime[2]) > 30 .OR. aTempoExec[nIniRot][1] <> aTempoExec[nFimRot][1]
	...
	cMsg := "A rotina de integra็ใo de Obras levou "+ELAPTIME( aTempoExec[nIniRot][2] , aTempoExec[nFimRot][2] )+" para executar."+cEnt
	...
	U_GenSendMail(,,,"noreply@grupogen.com.br",cEmailTi,Oemtoansi("Integra็ใo de Obras - Demora na execu็ใo"),cMsg,,,.F.)
EndIf
...
Do While !(cAlias)->(Eof())
	...
	*/
	...
	If AllTrim(cFab) = "378803" //Incluํda condi็ใo para alterar TES padrใo das obras GEN [Bruno Parreira,28/08/2019]
...
//		ALTERADO POR HELIMAR TAVARES - 14/12/2015 AS OBRAS AGORA ESTรO NO MAKER COM OS CAMPOS MEMO
...
*/		
		...
		// Cleuto -  17/09/2018 - incluida segunda unidade de medida para notas fiscais de exporta็ใo.
...
//			If !(cFiliOrig $ "0000-9022") //ALTERADO POR DANILO AZEVEDO - 21/08/2015 PARA NAO GERAR SALDO ORIGEM ATLAS
			If !(cFiliOrig $ "0000")//ALTERADO POR HELIMAR TAVARES - 14/12/2015 PARA GERAR SALDO ORIGEM ATLAS, POIS AS OBRAS AGORA ESTรO NO MAKER
				cFilAnt := cFiliOrig
