...
While (_cArqTmp)->(!EOF())
	...
	Do While cChavCli = alltrim(str((_cArqTmp)->C5_XPEDOLD))+" "+alltrim(str((_cArqTmp)->DIGITAL))
		...
		If Empty(_cMsg)
			...
			TMP_PDWEB->(DbGoTop())
			If TMP_PDWEB->(!EOF())
				cMsg :=	"Pedido web "+_cPedWeb+" já localizado na base de dados com o produto " + ALLTRIM((_cArqPro)->C6_PRODUTO)+"."+chr(13)+chr(10)+;
...
*/	
	...
	If nOpt <> 4 .Or. (nOpt = 4 .And. AllTrim(SA1->A1_XTIPCLI) == '020') 
...
User Function GENI018P(_aCabPd,_cPedOld,cFORMAPG,_nTotParc,_cMsg,nVldValPd,_cPedWeb,cOrigem)
