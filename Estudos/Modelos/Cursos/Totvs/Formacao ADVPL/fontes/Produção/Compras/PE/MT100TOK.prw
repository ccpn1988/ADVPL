...
If cTipo $ "B#D" .AND. cFilAnt <> "1022"
	...
	If AllTrim(cA100For) $ "0380795#0380796#0380794#031811#0378128#0005065" .AND. lContPd3 .AND. Posicione("SA1",1,xFilial("SA1")+cA100For+cLoja,"A1_CGC") <> SM0->M0_CGC
		...
		While TMPNFORI->(!EOF())			
			...
			If AllTrim(cSerie) == "4"/* quando serie 4 o item origem da SD2 pode nใo ser o mesmo da SD1 */
		...
		TMPNFORI->(DbEval( {|x| Aadd(aDados, { TMPNFORI->D2_COD,TMPNFORI->D2_ITEM,TMPNFORI->D2_NFORI,TMPNFORI->D2_SERIORI,TMPNFORI->D2_ITEMORI,TMPNFORI->F4_PODER3,TMPNFORI->F4_CODIGO } ) } ))
		TMPNFORI->(DbCloseArea())		
		...
		For nAuxIt := 1 To Len(aCols)
			...
			If aScan(aDados, {|x| ;
								...
								( AllTrim(x[ND2ITEMORI]) == AllTrim(GdFieldGet("D1_ITEMORI",nAuxIt,.F.,aHeader, aCols)) .OR. cSerie == "4" );
								} ) == 0 
