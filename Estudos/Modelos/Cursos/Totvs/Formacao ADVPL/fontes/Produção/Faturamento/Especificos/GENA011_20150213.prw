...
#DEFINE dProcInter cTod("31/01/2015")
...
If upper(alltrim(GetEnvServer())) $ "SCHEDULE" //EXECUTA SOMENTE NO AMBIENTE SCHEDULE
	...
	//dDataBase := cTod("31/01/2015") // 03/02 - RAFAEL LEITE
...
If DbSeek(xFilial("SA1")+PADR(AllTrim(_cMvClDe),TAMSX3("A1_COD")[1])+PADR(AllTrim(_cMvLjDe),TAMSX3("A1_LOJA")[1]))
	...
	If DbSeek(xFilial("SA2")+SA1->A1_CGC)
		...
		_cQuery += " AND SB1.B1_COD IN (SELECT D2_COD FROM SD2000 SD2 WHERE D_E_L_E_T_ = ' ' AND D2_FILIAL = '1022' AND D2_DOC = '000279982' AND D2_SERIE = '0')
...
_cQuery := " SELECT * FROM (
_cQuery += " SELECT DISTINCT SD2.D2_ITEM, SB1.B1_COD, SB1.B1_PROC, SB1.B1_LOJPROC, SB1.B1_DESC, SB1.B1_UM, SB1.B1_LOCPAD, SB1.B1_MSBLQL, D2_QUANT AS VALOR_ORI, 0 AS VALOR_DEV, 0 AS VALOR_CAN
_cQuery += " FROM SB1000 SB1, SD2000 SD2
_cQuery += " WHERE SD2.D_E_L_E_T_ = ' '
_cQuery += " AND SD2.D2_FILIAL = '1022'
_cQuery += " AND SD2.D2_DOC = '000279982'
_cQuery += " AND SD2.D2_SERIE = '0'
_cQuery += " AND SB1.D_E_L_E_T_ = ' '
_cQuery += " AND B1_COD = D2_COD
_cQuery += " )
_cQuery += " where VALOR_ORI <> 0 OR VALOR_DEV <> 0 OR VALOR_CAN <> 0
_cQuery += " ORDER BY B1_PROC, B1_LOJPROC, D2_ITEM
									...
									If DbSeek(xFilial("DA1")+_cMvTbPr+(_cAliQry)->B1_COD)
										...
										If _nPrcVn > 0
											...
											*/
											...
											_cQuery := "SELECT D2_FILIAL B6_FILIAL, D2_CLIENTE B6_CLIFOR, D2_LOJA B6_LOJA, D2_COD B6_PRODUTO, D2_NFORI B6_DOC, D2_QUANT B6_SALDO, D2_SERIORI B6_SERIE, D2_ITEMORI D1_ITEM, D2_IDENTB6 B6_IDENT
											_cQuery += " FROM SD2000 SD2
											_cQuery += " WHERE D_E_L_E_T_ = ' '
											_cQuery += " AND D2_FILIAL = '1022'
											_cQuery += " AND D2_DOC = '000279982'
											_cQuery += " AND D2_COD = '" + (_cAliQry)->B1_COD + "'
											_cQuery += " AND D2_SERIE = '0'
											...
											If !Empty(_cNotaImp)
												...
												*/
												...
												If TCSQLEXEC(_cQuery) != 0
													...
													//³Função para alimentar Log de erro³
													//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄI
													_cMsg := "1 - Não foi possivel realizar o update na SD2(Itens da Nota de Saida) após a execucao da Prestacao de Contas" + cEnt
													conout(Time()+ " " + _cMsg)
												...
												*/
												...
												If TCSQLEXEC(_cQuery) != 0
													...
													//³Função para alimentar Log de erro³
													//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄI
													_cMsg := "Não foi possivel realizar o update na SD1(Itens da Nota de Entrada) após a execucao da Prestacao de Contas" + cEnt
													conout(Time()+ " " + _cMsg)
													MemoWrite ( _cLogPd + "Emp" + _cEmp + " Fil" + _cFil + " " + STRTRAN(DTOC(ddatabase),"/","_") + " " + SUBSTR(STRTRAN(Time(),":",""),1,4) + "Consignação.txt" , _cMsg )
												EndIf
												...
												*/
												...
												If TCSQLEXEC(_cQuery) != 0
													...
													//³Função para alimentar Log de erro³
													//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄI
													_cMsg := "2 - Não foi possivel realizar o update na SD2(Itens da Nota de Entrada) Canceladas após a execucao da Prestacao de Contas" + cEnt
													conout(Time()+ " " + _cMsg)
													MemoWrite ( _cLogPd + "Emp" + _cEmp + " Fil" + _cFil + " " + STRTRAN(DTOC(ddatabase),"/","_") + " " + SUBSTR(STRTRAN(Time(),":",""),1,4) + "Consignação.txt" , _cMsg )
												EndIf
