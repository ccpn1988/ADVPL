...
Local _cParam1 := _aParam1[1] //Parametro enviado na configuração da schedule para saber se o TES a ser pesquisado é de Venda ou Oferta
...
Local _cTesDv	:= GetMv("GEN_FAT012") //Contém o TES utilizado no Pedido de Vendas das empresas Origem - Oferta
...
/*
LOG DE STATUS D2_XSTATUS E D1_XSTATUS:

V1: Prestação de Venda - Nota fiscal de saída do GEN para Origem
V2: Prestação de Venda - Nota fiscal de entrada na Origem (vinda do GEN)
V3: Prestação de Venda - Nota fiscal de saída da Origem para o GEN
V4: Prestação de Venda - Nota fiscal de entrada no GEN (vinda da Origem)
V5: Prestação de Venda - Nota fiscal de venda a mercado utilizada (considerada) na prestação de venda.
V6: Prestação de Venda - Nota fiscal de devolução de venda a mercado utilizada (considerada) na prestação de venda.
V7: Prestação de Venda - Nota fiscal de venda cancelada utilizada (considerada) na prestação de venda.

O1: Prestação de Oferta - Nota fiscal de saída do GEN para Origem
O2: Prestação de Oferta - Nota fiscal de entrada na Origem (vinda do GEN)
O3: Prestação de Oferta - Nota fiscal de saída da Origem para o GEN
O4: Prestação de Oferta - Nota fiscal de entrada no GEN (vinda da Origem)
O5: Prestação de Oferta - Nota fiscal de oferta a mercado utilizada (considerada) na prestação de venda.
O6: Prestação de Oferta - Nota fiscal de devolução de oferta a mercado utilizada (considerada) na prestação de venda.
O7: Prestação de Oferta - Nota fiscal de oferta cancelada utilizada (considerada) na prestação de venda.
*/
...
*/		
		...
		While (_cAliQry)->(!EOF())  
			...
			//Lote de execução
			_cUpLote	:= Soma1(GETMV("GEN_FAT074"))  
						...
						If DbSeek(xFilial("DA0")+_cMvTbPr)
							...
							If DA0->DA0_DATATE > dDatabase .OR. Empty(DA0->DA0_DATATE)
								...
								Else
									...
									//³Função para alimentar Log de erro³
									//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ Ù
									_cMsg := "Produto:  " + AllTrim((_cAliQry)->B1_COD) + " não possui saldo em poder de terceiros, para o cliente: " + _cCliPv + " e loja: " + _cLjPv
									conout(Time()+ " " + _cMsg)
									MemoWrite ( _cLogPd + "Emp" + _cEmp + " Fil" + _cFil + " " + STRTRAN(DTOC(ddatabase),"/","_") + " " + SUBSTR(STRTRAN(Time(),":",""),1,4) + " Prod"  + AllTrim((_cAliQry)->B1_COD) + ".txt" , _cMsg )
									(_cAliQry)->(DbSkip())
								EndIf
