...
If File("\spool\GENA084_"+cPedido+".log")
...
cQuery += " SELECT C5_TPFRETE,A4_COD||'-'||TRIM(A4_NOME) TRANSP,E4_DESCRI CODPAG,C5_FRETE,TRIM(A3_NOME) A3_NOME,TRIM(F4_TEXTO)||' - '||TRIM(F4_FINALID) TES,B1_ISBN,C5_XPEDCLI,ZZB_EMISSA,A1_COD||A1_LOJA A1_COD,A1_NREDUZ,ZZB_QTDVEN,"+chr(13)+chr(10) 
		...
		Do Case
			Case nSitAut == 0 .OR. nSitAut == 1
				...
				If File("\spool\GENA084_"+cPedido+".log")
					...
					oSecLog:Init()
					FT_FUSE("\spool\GENA084_"+cPedido+".log")
					FT_FGOTOP()
					While ( !FT_FEOF() )
...
If lImpLog .AND. File("\spool\GENA084_"+cPedido+".log")
	oSecLog := oReport:Section(5)
	oSecLog:Init()
	FT_FUSE("\spool\GENA084_"+cPedido+".log")
	FT_FGOTOP()
