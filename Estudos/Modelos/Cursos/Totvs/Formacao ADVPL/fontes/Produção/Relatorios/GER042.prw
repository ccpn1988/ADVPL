...
ฑฑบPrograma  ณGER042    บAutor  ณErica Vieites       บ Data ณ  03/02/2016 บฑฑ
ฑฑฬออออออออออุออออออออออสอออออออฯออออออออออออออออออออสออออออฯอออออออออออออนฑฑ
ฑฑบDesc.     ณRelatorio de Oferta Produtos Impressos CRM                  บฑฑ
ฑฑบ          ณ                                                            บฑฑ
...
oReport := TReport():New("GER042","OFERTA PRODUTOS IMPRESSOS CRM",cPerg,{|oReport| PrintReport(oReport)},"OFERTA PRODUTOS IMPRESSSOS CRM")
...
//Secao do relatorio
oSection1 := TRSection():New(oReport,"Oferta Produtos Impressos","SD2")
...
TRCell():New(oSection1,"CUSTO"	    	," ","Custo"+CRLF+"Unit.",,15,,,,,"RIGHT")        
TRCell():New(oSection1,"CSTTOT"	    	," ","Custo"+CRLF+"Total",,15,,,,,"RIGHT")        
...
_cParm3 := DTOS(LASTDATE(MV_PAR02))
...
Begin Report Query oSection1
	...
	FROM 
	(SELECT CLIENTE,LOJA,PROD,NOTA,SERIE,FILIAL, SUM(QTDE) QTDE, SUM(LIQUIDO) LIQ 
	...
	HAVING SUM(QTDE) <> 0) O 
	JOIN %table:SA1% SA1 ON O.CLIENTE = SA1.A1_COD
	   AND O.LOJA = SA1.A1_LOJA
	   AND SA1.A1_FILIAL = %xFilial:SA1%
	   AND SA1.%notDel% 
	JOIN %table:SB1% SB1 ON O.PROD = SB1.B1_COD
	   AND SB1.B1_FILIAL = %xFilial:SB1%
	   AND SB1.%notDel% 
	JOIN %table:SB5% SB5 ON SB1.B1_COD = SB5.B5_COD
	   AND SB5.B5_FILIAL = %xFilial:SB5%
	   AND SB5.%notDel% 
	JOIN %table:SX5% SX5 ON SX5.X5_CHAVE = SB1.B1_XIDTPPU
	  AND SX5.X5_TABELA = 'Z4'
	  AND TRIM(SX5.X5_DESCENG) = '0'
	...
	ON SBM.BM_GRUPO = SB1.B1_GRUPO
	 ...
	 AND SBM.%notDel% 
	LEFT JOIN (SELECT SB9.B9_COD, MAX(SB9.B9_CM1) CUSTO
	             FROM %table:SB9% SB9, %table:SB1% SB1
	            WHERE SB9.B9_COD = SB1.B1_COD
	              AND SB9.B9_FILIAL = DECODE(SB1.B1_PROC,'0380795','2022','0380796','3022','0380794','4022','031811 ','6022', '0378128', '9022')
	              AND SB9.B9_DATA = %Exp:_cParm3%
	              AND SB9.%notDel%
	              AND SB1.%notDel%
	            GROUP BY SB9.B9_COD) SB9
