...
Static Function Import( cAlias, aCampos, cFonte )
	...
	DBSELECTAREA(cAlias)
	dbsetorder(1)
	msSeek(xFilial(cAlias)+aCampos[1]+aCampos[2])
	
	oModel := FWLoadModel( cFonte )
	oModel:SetOperation( 4 ) //( MODEL_OPERATION_INSERT )
	oModel:Activate()
	oAux := oModel:GetModel( "MATA020_SA2" )// ID do componete addfields
    oAux:GetStruct():SetProperty('*',MODEL_FIELD_WHEN , FWBuildFeature( STRUCT_FEATURE_WHEN , NIL   ) )
    
    //Desativando validaçao
    oAux:GetStruct():SetProperty('A2_COD',MODEL_FIELD_VALID, FWBuildFeature( STRUCT_FEATURE_VALID,'.T.' ) ) 	   
	oAux:GetStruct():SetProperty('*',MODEL_FIELD_VALID, FWBuildFeature( STRUCT_FEATURE_WHEN, NIL ) )   
	aAux := oAux:GetStruct():GetFields()
	
	aAux := oAux:GetStruct():GetFields()
	
	For nI := 3 To Len( aCampos )
...
If lRet
// neste momento os dados não são gravados, são somente validados. = TUDOOK
