...
/*
Exerc. correção dos erros:

Totalizador  do grid
Erro de relacionemento SZ3não grava o numero do pedido correto
Gravando registro em branco na SZ3
Error log: variable does not exist ALIAS2 on MOD3ACOLS(xmodelo3.prw)
Excluir: Exclui a SZ2, mas não exclui o SZ3
*/
...
Local nI := 0
		...
		For nX := 1 To Len( aCOLS )
			If ! aCols[nX][Len(aHeader)+1]       // Valida se a linha esta deletada
				...
				RecLock( cAlias2, .T. )
					...
					SZ3->Z3_COD    := M->Z2_COD   //toda vez que trabalhar com incluir - trabalhar com a memoria
				MsUnLock()
			Endif
		Next nX
	...
	If nOpc == 5 // EXCLUSãO
	  BEGIN TRANSACTION
		// Deleta os Itens
		dbSelectArea(cAlias2)//SZ3
		dbSetOrder(1)

		dbSeek(xFilial(cAlias2) + SZ2->Z2_COD)  //OU M->Z2_COD 
