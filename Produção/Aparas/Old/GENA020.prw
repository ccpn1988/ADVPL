...
±±ºPrograma  ³GENA020   ºAutor  ³Danilo Azevedo      º Data ³  15/06/15   º±±
...
User Function GENA020()

conout(Time()+ " " + "GENA020 - Inicio - Rotina de aparas")
Prepare Environment Empresa "00" Filial "1022"

U_GENA020A()

conout(Time()+ " " + "GENA020 - Fim - Rotina de aparas")
Reset Environment
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±ÉÍÍÍÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍ»±±
±±ºPrograma  ³GENA020A  ºAutor  ³Angelo Henrique     º Data ³  29/08/14   º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
...
conout(Time()+ " " + "GENA020A - Inicio - Verificando Itens da Nota de Entrada")
...
Else
	...
	Else
		...
		_cQuery += "     WHERE I11.VIEW_NAME LIKE '%GENA020_OBRA_NO_PROC%' "
 ...
 /*Saldo a ser gerado na Origem: */	" | "+AllTrim(Str(  IIF( (_cAliQry5)->SALDO_ORI-(_cAliQry)->B2_QATU < 0 , ((_cAliQry5)->SALDO_ORI-(_cAliQry)->B2_QATU)*(-1), (_cAliQry5)->SALDO_ORI-(_cAliQry)->B2_QATU )))
					...
					*/
					...
					For _ni := 1 To Len(_aSldB6)
						...
						aAdd(_alinhaOr	,	{"D1_LOCAL"		, "03"									, Nil})
						/*
			...
			If Len(_aItmPv) > 0
				...
				If !_lVcDesc
					...
					_cNtDvCon := U_GENA020B(_aCabPv,_aItmPv)
					...
					If !Empty(_cNtDvCon)
						...
						conout(Time()+ " " + "GENA020 - Inicio do RPC para logar na empresa origem Pedido/Nota de Saída")
						conout(Time()+ " " + "GENA020 - Empresa: " + _cEmpCd + " Filial: " + _cEmpFl)
						...
						If ValType(_oServer) == "O"
							...
							_cNotaImp := ""
							_cNotaImp := _oServer:CallProc("U_GENA020C",_aCabPd,_cTemp1,_aCabDcOr,_cTemp2,_cNtDvCon,dDatabase)
							//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
							//³Realizando a nova conexão para entrar na empresa e filial correta³
							//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
							//Fecha a Conexao com o Servidor
							...
							_oServer := Nil
						Else
							conout(Time()+ " " + "GENA020 - Nao foi possivel logar. Retorno para empresa origem nao executado.")
						EndIf
...
conout(Time()+ " " + "GENA020A - Fim - Verificando Itens da Nota de Entrada")
RestArea(_aArea)
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±ÉÍÍÍÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍ»±±
±±ºPrograma  ³GENA020B  ºAutor  ³Angelo Henrique     º Data ³  17/09/14   º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
...
User Function GENA020B(_aCabPv,_aItmPv)
...
conout(Time()+ " " + "GENA020B - Rotina para execução do Execauto de Geração do Pedido de Vendas e de Geração do Documento de Saída, empresa Matriz (GEN)")
...
If !lMsErroAuto
	...
	conout(Time()+ " " + "GENA020B - Gerou com sucesso o pedido, irá ver se existe a necessidade de desbloquear por crédito na empresa Matriz (GEN)")
	...
	If DbSeek(xFilial("SC9") + SC5->C5_NUM)
		...
		conout(Time()+ " " + "GENA020B - Ira varrer a SC9 para realizar o desbloqueio por crédito na empresa Matriz (GEN)")
		...
		While ( (_cAliSC9)->(!Eof()) .And. (_cAliSC9)->C9_FILIAL == xFilial("SC9") .And. (_cAliSC9)->C9_PEDIDO == SC5->C5_NUM .And. Eval(bValid) )
			If !( (Empty(SC9->C9_BLCRED) .And. Empty(SC9->C9_BLEST)) .OR. (SC9->C9_BLCRED=="10" .And. SC9->C9_BLEST=="10") .OR.;
				SC9->C9_BLCRED=="09" )
				conout(Time()+ " " + "GENA020B - Liberação de Crédito do Pedido de Vendas na empresa Matriz (GEN)")
	...
	If DbSeek(xFilial("SC9")+SC5->C5_NUM)
		...
		conout(Time()+ " " + "GENA020B - Inicio da Geração do Documento de Saída na empresa Matriz (GEN).")
		...
		conout(Time()+ " " + "GENA020B - Ira realizar a geracao da nota de saida na empresa Matriz (GEN) ")
		...
		*'---------------------------------------------------------'*
		...
		If Empty(AllTrim(_cNotaImp))
			conout(Time()+ " " + "GENA020B - Geração do Documento de Saída na empresa Matriz (GEN) apresentou erro .")
			//_cPedExc := SC9->C9_PEDIDO
			...
			conout(Time()+ " " + "GENA020B - Irá alterar o pedido de vendas para poder realizar a exclusão na empresa Matriz (GEN)")
			
			lMsErroAuto := .F.
			MSExecAuto({|x,y,z| Mata410(x,y,z)},_aCabPv,_aItmPv,4)
			
			If !lMsErroAuto
				conout(Time()+ " " + "GENA020B - Alterou o pedido de vendas com sucesso, irá realizar a exclusão na empresa Matriz (GEN).")
				
				lMsErroAuto := .F.
				MSExecAuto({|x,y,z| Mata410(x,y,z)},_aCabPv,_aItmPv,5)
				
				If !lMsErroAuto
					conout(Time()+ " " + "GENA020B - Excluiu com sucesso o pedido de vendas na empresa Matriz (GEN).")
				...
				Else
					conout(Time()+ " " + "GENA020B - Não conseguiu excluir o pedido de vendas na empresa Matriz (GEN).")
				...
				EndIf
			Else
				conout(Time()+ " " + "GENA020B - Não conseguiu alterar o pedido de vendas na empresa Matriz (GEN).")
	...
	EndIf
Else
	conout(Time()+ " " + "GENA020B - Não conseguiu gerar o Pedido de Vendas na empresa Matriz (GEN). ")
	_aErro := GetAutoGRLog()
	For _ni := 1 To Len(_aErro)
		_cErroLg += _aErro[_ni] + cEnt
		conout(_cErroLg)
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±ÉÍÍÍÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍ»±±
±±ºPrograma  ³GENA020C  ºAutor  ³Angelo Henrique     º Data ³  17/09/14   º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
...
ßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßß
*/
User Function GENA020C(_aCabPd,_cTemp1,_aCabDcOr,_cTemp2,_cNtDvCon,dProcPed)
...
conout(Time()+ " " + "GENA020C - Rotina de Geração da Nota Fiscal de Entrada (Devolução da Consignação) e Geração da Nota Fiscal de Saída da Venda na empresa Origem ")
conout(Time()+ " " + "GENA020C - Primeiro a Geração da Nota Fiscal de Entrada (Devolução da Consignação) da empresa origem")
...
If lMsErroAuto
	cMsg := "GENA020 - VENDA APARAS" + cEnt
	cMsg += cEnt
	...
	_lRet := .F.
	conout(Time()+ " " + "GENA020C - Não conseguiu gerar a Nota Fiscal de Entrada (Devolução da Consignação) na empresa origem. ")
	_aErro := GetAutoGRLog()
	_cErroLg	:= ""
	For _ni := 1 To Len(_aErro)
		_cErroLg += _aErro[_ni] + cEnt
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±ÉÍÍÍÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍ»±±
±±ºPrograma  ³GENA020   ºAutor  ³Microsiga           º Data ³  08/12/16   º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºDesc.     ³                                                            º±±
±±º          ³                                                            º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
...
If !Empty(cLogSld)
	cMsg := "GENA020 - TRANSFERENCIA APARAS" + cEnt
	cMsg += cLogSld
...
If lTransf
	...
	cMsg := "GENA020 - TRANSFERENCIA APARAS" + cEnt
	cMsg += cEnt
	cMsg += "Realizada transferencia do documento "+_cNtDvCon+"/"+_cMvSeri+" Armazem 01 para 03 - "+SM0->M0_CODFIL+" - "+alltrim(SM0->M0_NOMECOM)
...
for _nIt := 1 to len(_aItExcel)     
	...
	if valtype(_oFWMsExcel) == 'U' 
    	...
    	*/
        ...
        _oFWMsExcel:AddColumn("Erros","Aparas","TITULO",1,1)
        _oFWMsExcel:AddColumn("Erros","Aparas","TIPO PUB.",1,1)                
        _oFWMsExcel:AddColumn("Erros","Aparas","FORNECEDOR",1,1)        
        _oFWMsExcel:AddColumn("Erros","Aparas","NOME FORN.",1,1)                
	...
	Endif           
 	...
 	cTitulo	:= AllTrim(Posicione("SB1",1,xFilial("SB1")+Alltrim(_aItExcel[_nIt][ _CINDICE  ] ),"B1_DESC"))
 	cTpPub		:= Posicione("SX5",1,xFilial("SX5")+"Z4"+SB1->B1_XIDTPPU,"X5_DESCRI")
 	cNomeFo	:= Posicione("SA2",1,xFilial("SA2")+SB1->B1_PROC+SB1->B1_LOJPROC,"A2_NOME")
