...
±±ºPrograma  ³GENA070   ºAutor  ³Danilo Azevedo      º Data ³  15/06/15   º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºDesc.     ³Rotina de Aparas para processar obras sem saldo em poder 3  º±±
±±º          ³                                                            º±±
...
User Function GENA070()

conout(Time()+ " " + "GENA070 - Inicio - Rotina de aparas")
Prepare Environment Empresa "00" Filial "1022"

LockByName("GENA070",.T.,.T.,.T.)
U_GENA070A("03")
UnLockByName("GENA070",.T.,.T.,.T.)

conout(Time()+ " " + "GENA070 - Fim - Rotina de aparas")
Reset Environment

Return()

User Function GENA070D()
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±ÉÍÍÍÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍ»±±
±±ºPrograma  ³GENA070A  ºAutor  ³Angelo Henrique     º Data ³  29/08/14   º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
...
Local _cLogPd	:= GetMv("GEN_FAT079") //Contém o caminho que será gravado o log de erro
...
Local _cMvTsPd	:= GetMv("GEN_FAT173") //Contém o TES utilizado no Pedido de Vendas das empresas Matriz, consignação (positivo)
Local _cMvTsOr	:= GetMv("GEN_FAT174") //Contém o TES utilizado na nota de entrada das empresas Origem, consignação (positivo) - Vendas
Local _cServ 	:= "10.3.0.85"//GETMV("GEN_FAT027") //Contém o Ip do servidor para realizar as mudanças de ambiente
Local _nPort  	:= 1050 //GETMV("GEN_FAT028") //Contém a porta para realizar as mudanças de ambiente
...
conout(Time()+ " " + "GENA070A - Inicio - Verificando Itens da Nota de Entrada")
...
Else
	...
	If !DbSeek(xFilial("SA2")+SA1->A1_CGC)
		...
		// Tabela, descritivo, indice, mensagem, valores, código do produto.
		...
		*/
		...
		While (_cAliQry)->(!EOF())
			...
			If AllTrim((_cAliQry)->B1_PROC) == "378803"
				_cMsg := "Obra com fornecedor GEN, não será processada: " + (_cAliQry)->B1_PROC + " e loja: " + (_cAliQry)->B1_LOJPROC  + ", vinculados ao produto: " + (_cAliQry)->B1_COD
				conout(Time()+ " " + _cMsg)
			...
			If AllTrim((_cAliQry)->B1_PROC) == "031811"
				_cMsg := "Obra com fornecedor ACF GEN, não será processada: " + (_cAliQry)->B1_PROC + " e loja: " + (_cAliQry)->B1_LOJPROC  + ", vinculados ao produto: " + (_cAliQry)->B1_COD
				conout(Time()+ " " + _cMsg)
   			   aAdd ( _aLog , { cEmpAnt + '-'+cFilAnt , "SA2" , 'Fornecedor',(_cAliQry)->B1_PROC+(_cAliQry)->B1_LOJPROC ,_cMsg, 0,0,0,0,0,0,0,0,0, (_cAliQry)->B1_COD } )    
			...
			If !DbSeek(xFilial("DA0")+_cMvTbPr)
				...
				// Tabela, descritivo, indice, mensagem, valores
   				aAdd ( _aLog , { cEmpAnt + '-'+cFilAnt , "DA0" ,'Preços', _cMvTbPr ,_cMsg, 0,0,0,0,0,0,0,0,0, ' ' } )       
			...
			_nItMax := 1
			While AllTrim((_cAliQry)->B1_PROC) == _cForn .And. AllTrim((_cAliQry)->B1_LOJPROC) == _cLojFn .and. _nItMax < 90
				...
				If _nPrcVn > 0
					...
					_cQuery += " B6_SERIE, B6_CLIFOR, B6_LOJA, D1_ITEM, B6_IDENT,(D1_VALDESC/D1_TOTAL) DESCONTO,(D1_TOTAL-D1_VALDESC)/D1_QUANT PRC_CALC,B6_QUANT
					...
					While (_cAliQry5)->(!EOF())
						...
						_nPruNit := ROUND(IIF( (_cAliQry5)->DESCONTO <> 0, (_cAliQry5)->PRC_CALC , (_cAliQry5)->B6_PRUNIT ),2)
						aAdd(_aSldB6,{_nQtdSld, (_cAliQry5)->B6_DOC, (_cAliQry5)->B6_SERIE, " "				, " ", _nPruNit, " " })
						Exit
...
//						aAdd(_alinhaOr	,	{"D1_IDENTB6"	, PADR(AllTrim(_aSldB6[_ni][7]),TAMSX3("C6_IDENTB6")[1]) , Nil}) // 03/02 - RAFAEL LEITE										                        
				...
				Else
					...
					// Empresa/Filial, Tabela, descritivo, indice, mensagem, valores                                                      
	  				aAdd ( _aLog , { cEmpAnt + '-'+cFilAnt ,"DA1" ,"Itens da Tabela de Preços",  _cMvTbPr+(_cAliQry)->B1_COD ,_cMsg, 0,0,0,0,0,0,0,0,0,(_cAliQry)->B1_COD } )    
			...
			If Len(_aItmPv) > 0
				...
				If !_lVcDesc
					...
					_cNtDvCon := U_GENA070B(_aCabPv,_aItmPv)
					...
					If !Empty(_cNtDvCon)
						...
						conout(Time()+ " " + "GENA070 - Inicio do RPC para logar na empresa origem Pedido/Nota de Saída")
						conout(Time()+ " " + "GENA070 - Empresa: " + _cEmpCd + " Filial: " + _cEmpFl)
						...
						If ValType(_oServer) == "O"
							...
							_cNotaImp := ""
							_cNotaImp := _oServer:CallProc("U_GENA070C",_aCabPd,_cTemp1,_aCabDcOr,_cTemp2,_cNtDvCon,dDatabase,uLocal)
							//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
							//³Realizando a nova conexão para entrar na empresa e filial correta³
							//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
							//Fecha a Conexao com o Servidor
							...
							_oServer := Nil
						Else
							conout(Time()+ " " + "GENA070 - Nao foi possivel logar. Retorno para empresa origem nao executado.")
						EndIf
...
// Gerar Excel com possíveis LOG  - Ivan (Loop) em 20/10/2017
if  !Empty(_aLog)
...
conout(Time()+ " " + "GENA070A - Fim - Verificando Itens da Nota de Entrada")
RestArea(_aArea)
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±ÉÍÍÍÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍ»±±
±±ºPrograma  ³GENA070B  ºAutor  ³Angelo Henrique     º Data ³  17/09/14   º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
...
User Function GENA070B(_aCabPv,_aItmPv)
...
conout(Time()+ " " + "GENA070B - Rotina para execução do Execauto de Geração do Pedido de Vendas e de Geração do Documento de Saída, empresa Matriz (GEN)")
...
If !lMsErroAuto
	...
	conout(Time()+ " " + "GENA070B - Gerou com sucesso o pedido, irá ver se existe a necessidade de desbloquear por crédito na empresa Matriz (GEN)")
	...
	If DbSeek(xFilial("SC9") + SC5->C5_NUM)
		...
		conout(Time()+ " " + "GENA070B - Ira varrer a SC9 para realizar o desbloqueio por crédito na empresa Matriz (GEN)")
		...
		While ( (_cAliSC9)->(!Eof()) .And. (_cAliSC9)->C9_FILIAL == xFilial("SC9") .And. (_cAliSC9)->C9_PEDIDO == SC5->C5_NUM .And. Eval(bValid) )
			If !( (Empty(SC9->C9_BLCRED) .And. Empty(SC9->C9_BLEST)) .OR. (SC9->C9_BLCRED=="10" .And. SC9->C9_BLEST=="10") .OR.;
				SC9->C9_BLCRED=="09" )
				conout(Time()+ " " + "GENA070B - Liberação de Crédito do Pedido de Vendas na empresa Matriz (GEN)")
	...
	If DbSeek(xFilial("SC9")+SC5->C5_NUM)
		...
		conout(Time()+ " " + "GENA070B - Inicio da Geração do Documento de Saída na empresa Matriz (GEN).")
		...
		conout(Time()+ " " + "GENA070B - Ira realizar a geracao da nota de saida na empresa Matriz (GEN) ")
		...
		*'---------------------------------------------------------'*
		...
		Elseif !_lEspec
			...
			// Empresa/Filial, Tabela, descritivo, indice, mensagem, valores                                                      
	  		aAdd ( _aLog , { cEmpAnt + '-'+cFilAnt ,"SF2" ,"Cabeçalho das NF Saída",  _cPedExc,_cMsg, 0,0,0,0,0,0,0,0,0, ' ' } )    
		...
		If Empty(AllTrim(_cNotaImp))
			conout(Time()+ " " + "GENA070B - Geração do Documento de Saída na empresa Matriz (GEN) apresentou erro .")
			//_cPedExc := SC9->C9_PEDIDO
			...
			conout(Time()+ " " + "GENA070B - Irá alterar o pedido de vendas para poder realizar a exclusão na empresa Matriz (GEN)")
			
			lMsErroAuto := .F.
			MSExecAuto({|x,y,z| Mata410(x,y,z)},_aCabPv,_aItmPv,4)
			
			If !lMsErroAuto
				conout(Time()+ " " + "GENA070B - Alterou o pedido de vendas com sucesso, irá realizar a exclusão na empresa Matriz (GEN).")
				
				lMsErroAuto := .F.
				MSExecAuto({|x,y,z| Mata410(x,y,z)},_aCabPv,_aItmPv,5)
				
				If !lMsErroAuto
					conout(Time()+ " " + "GENA070B - Excluiu com sucesso o pedido de vendas na empresa Matriz (GEN).")
				...
				Else
					conout(Time()+ " " + "GENA070B - Não conseguiu excluir o pedido de vendas na empresa Matriz (GEN).")
				...
				EndIf
			Else
				conout(Time()+ " " + "GENA070B - Não conseguiu alterar o pedido de vendas na empresa Matriz (GEN).")
	...
	EndIf
Else
	conout(Time()+ " " + "GENA070B - Não conseguiu gerar o Pedido de Vendas na empresa Matriz (GEN). ")
	_aErro := GetAutoGRLog()
	For _ni := 1 To Len(_aErro)
		_cErroLg += _aErro[_ni] + cEnt
		conout(_cErroLg)
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±ÉÍÍÍÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍ»±±
±±ºPrograma  ³GENA070C  ºAutor  ³Angelo Henrique     º Data ³  17/09/14   º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
...
ßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßß
*/
User Function GENA070C(_aCabPd,_cTemp1,_aCabDcOr,_cTemp2,_cNtDvCon,dProcPed,uLocal)
...
Local _nPosLb			:= 0
Local _cPedExc			:= ""
Local _cLogPd			:= GetMv("GEN_FAT079")+DtoS(DDataBase)+"\"//Contém o caminho que será gravado o log de erro
Local _cMvSeri 			:= GetMv("GEN_FAT026") //SERIE nota de saída
Local _cEmp				:= AllTrim(SM0->M0_CODIGO)
Local _cFil				:= AllTrim(SM0->M0_CODFIL)
Local _cMvEsp			:= GetMv("MV_ESPECIE") //Contem tipos de documentos fiscais utilizados na emissao de notas fiscais
...
conout(Time()+ " " + "GENA070C - Rotina de Geração da Nota Fiscal de Entrada (Devolução da Consignação) e Geração da Nota Fiscal de Saída da Venda na empresa Origem ")
conout(Time()+ " " + "GENA070C - Primeiro a Geração da Nota Fiscal de Entrada (Devolução da Consignação) da empresa origem")
...
If lMsErroAuto
	cMsg := "GENA070 - VENDA APARAS" + cEnt
	cMsg += cEnt
	...
	_lRet := .F.
	conout(Time()+ " " + "GENA070C - Não conseguiu gerar a Nota Fiscal de Entrada (Devolução da Consignação) na empresa origem. ")
	_aErro := GetAutoGRLog()
	_cErroLg	:= ""
	For _ni := 1 To Len(_aErro)
		_cErroLg += _aErro[_ni] + cEnt
	...
	// Empresa/Filial, Tabela, descritivo, indice, mensagem, valores                                                      
	aAdd ( _aLog , { _cEmp + '-'+_cFil ,"SF1" ,"Cabeçalho das nf de Entrada",  _cNtDvCon+"/"+_cMvSeri+" - "+SM0->M0_CODFIL+" - "+alltrim(SM0->M0_NOMECOM), 0,0,0,0,0,0,0,0,0, ' ' } )    
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±ÉÍÍÍÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍ»±±
±±ºPrograma  ³GENA070   ºAutor  ³Microsiga           º Data ³  08/12/16   º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºDesc.     ³                                                            º±±
±±º          ³                                                            º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
...
Local lRet		:= .F.
Local cLogPd	:= AllTrim(GetMv("GEN_FAT079"))+"LogTransf\"+DtoS(DDataBase)+"\" //Contém o caminho que será gravado o log de erro
...
If !Empty(cLogSld)
	cMsg := "GENA070 - TRANSFERENCIA APARAS" + cEnt
	cMsg += cLogSld
	...
	// Empresa/Filial, Tabela, descritivo, indice, mensagem, valores                                                      
	aAdd ( _aLog , { cEmpAnt + '-'+cFilAnt ,"SF1xSF2" ,"Cabeçalho das nf de Entrada/Saída",  AllTrim(_cNtDvCon)+"_"+AllTrim(_cMvSeri)+"_"+AllTrim(_aCabDcOr[nPFor][2])+"_"+AllTrim(_aCabDcOr[nPcli][2]), 0,0,0,0,0,0,0,0,0, ' ' } )    
...
If Len(aTransfer) == 0
	...
	cMsg += "Nota "+_cNtDvCon+" não transferida para o "+uLocal+" na origem" + cEnt
	...
	Begin Transaction
		...
		If lMsErroAuto   				
		   ...
		   //	MemoWrite(cLogPd+"Emp"+"_"+cFilAnt+"_"+DTOS(ddatabase)+ "_" + SUBSTR(STRTRAN(Time(),":",""),1,4)+"_"+Padr(AllTrim(_cNtDvCon),TamSX3("F1_DOC")[1])+"_"+Padr(AllTrim(_cMvSeri),TamSX3("F1_SERIE")[1])+"_"+Padr(AllTrim(_aCabDcOr[nPFor][2]),TamSX3("F1_FORNECE")[1])+"_"+Padr(AllTrim(_aCabDcOr[nPcli][2]),TamSX3("F1_LOJA")[1])+"_Transferencia.log",cMsg)
		   	...
		   	// Empresa/Filial, Tabela, descritivo, indice, mensagem, valores                                                      
			aAdd ( _aLog , {  cEmpAnt + '-'+cFilAnt ,"SF1xSF2" ,"Cabeçalho das nf de Entrada/Saída",  _cMensTxt, 0,0,0,0,0,0,0,0,0, ' ' } )    
		...
		//		cUpdAux += " AND D3_LOCAL = '"+aChave[nMvtAux][4]+"'"
				...
				If (TCSqlExec(cUpdAux) < 0)
					...
					// Empresa/Filial, Tabela, descritivo, indice, mensagem, valores                                                      
					aAdd ( _aLog , { cEmpAnt + '-'+cFilAnt ,"SD3" ,"Movimentações internas", _cNtDvCon+"/"+_cMvSeri+" - "+SM0->M0_CODFIL+" - "+alltrim(SM0->M0_NOMECOM), 0,0,0,0,0,0,0,0,0, ' ' } )    
...
Gerar Planilha Excel de possíveis LOG - Rotina de Aparas 
@type 		Static  function
@author 	Ivan de Oliveira - Loop
@since 	20/10/2017
@version 	1.0
@Param  	${_aItExcel}, ${Array}  		- Array contendo colunas com erros   
@return 	${_cLogPd}, ${Caractere}  	- Local onde será gerado o arquivo.
@example 
...
for _nIt := 1 to len(_aItExcel)     
	...
	// Próximo alias para tabela temporária  
	...
	if !Empty(_cProd)   
		...
		if  _nPos == 0  
			...
			// Buscando informações SALDO GEN
	...
	if valtype(_oFWMsExcel) == 'U' 
    	...
    	*/
        ...
        _oFWMsExcel:AddColumn("Erros","Aparas","DESCRIÇÃO",1,1)
        _oFWMsExcel:AddColumn("Erros","Aparas","ÍNDICE",1,1)
