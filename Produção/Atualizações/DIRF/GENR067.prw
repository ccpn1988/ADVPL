...
IF nModel == 2
	oCodRet := TComboBox():New(16, 135,{|u|if(PCount()>0,cTipo:=u,cTipo)},{" ","1=5952", "2=1708","3=0588","4=0422"},30,30,oDlg,,{|| },,,,.T.,,,,,,,,,'cTipo','C๓d.Ret.: ',1,oFont,CLR_RED)
...
If nOpca == 1
	...
	IF nModel == 1
		Processa({|| ProcBase(cAnoDirf) },"Analisando base, aguarde...")
	Else	
...
xAddSheet(cSheet,cTable,oExcel,cAliTmp1)
/**************************************************************************************/
// Cleuto - 28/12/2020 - inclusใo do codigo de reten็ใo 0422
cSheet	:= "Cod.0422 e Forn. Difente de EX."
cTable	:= "Reten็ใo Cod.0422 e Forne. Diferente de Exterior (A2_CALCIRF <> 2)"

IncProc("Analisando "+cSheet)
cAliTmp1	:= xSqlTemp11(cAnoDirf)

xAddSheet(cSheet,cTable,oExcel,cAliTmp1)
/**************************************************************************************/
// Cleuto - 28/12/2020 - inclusใo do codigo de reten็ใo 0422
cSheet	:= "Forn.Ex. e Tit.TX Difer.0422"
cTable	:= "Fornecedor Ex. e Tit.TX Cod. diferente de 0422"

IncProc("Analisando "+cSheet)
cAliTmp1	:= xSqlTemp12(cAnoDirf)
...
BeginSql Alias cAliTmp 
	...
	AND A2_TIPO = 'F'
	AND E2_TIPO <> 'TX'
	AND E2_CODRET NOT IN('0422','8045')
	AND SED.ED_CALCIRF = 'S'
...
Static Function xSqlTemp11(cAno)
...
BeginSql Alias cAliTmp 
	...
	CASE WHEN A2_RECPIS = '1' THEN 'SIM' ELSE 'NรO' END A2_RECPIS /*"For.Recolhe PIS"*/,
	CASE WHEN A2_RECCSLL = '1' THEN 'SIM' ELSE 'NรO' END A2_RECCSLL /*"For.Recolhe CSLL"*/,
	CASE WHEN A2_CALCIRF = '1' THEN 'SIM' WHEN A2_CALCIRF = '2' THEN 'NA BAIXA' ELSE 'NรO' END A2_CALCIRF /*"For.Calcula IR"*/,
	/* CONFIGURAวรO DE CALCULO DO PCC/IR NA NATUREZA */
	CASE WHEN ED_CALCPIS = 'S' THEN 'SIM' ELSE 'NรO' END ED_CALCPIS /*"Nat.Calcula PIS"*/,
	ED_PERCPIS /*"Nat.Percentual de PIS"*/,
	CASE WHEN ED_CALCCOF = 'S' THEN 'SIM' ELSE 'NรO' END ED_CALCCOF /*"Nat.Calcula COFINS"*/,
	...
	WHERE SE2.%NotDel%
	AND SE2.E2_BAIXA BETWEEN %Exp:cDtIni% AND %Exp:cDtFim%	
	/* verifico se existe TX 0422 */
	AND (SELECT COUNT(*) 
      FROM %Table:SE2% TX
      WHERE TX.E2_FILIAL = RPAD(SE2.E2_FILIAL,4)
      AND TX.E2_TITPAI = RPAD(SE2.E2_PREFIXO||SE2.E2_NUM||SE2.E2_PARCELA||SE2.E2_TIPO||SE2.E2_FORNECE||SE2.E2_LOJA,50)
      AND TX.E2_NATUREZ IN ('IRF       ')
      AND ( TX.E2_CODRET = '0422' )
      AND TX.%NotDel% ) <> 0
	/* verifico se o cadastro do fornecedor estแ corretamente cadastrado */
	AND ( A2_TIPO <> 'X' OR A2_CALCIRF <> '2' )
...
Static Function xSqlTemp12(cAno)
...
cQuery+="    WHEN A2_TIPO = 'X' AND A2_CALCIRF = '2' THEN 'ESTRANGEIRO'  "
cQuery+="    WHEN A2_TIPO = 'X' THEN 'OUTROS'  "
cQuery+="    ELSE A2_TIPO  "
cQuery+="  END A2_TIPO"
cQuery+="  ,A2_NOME,A2_COD,A2_LOJA,"

If nTipo == 4 .OR. nTipo == 5
	cQuery+="  A2_COD||A2_LOJA A2_CGC
Else
	cQuery+="  A2_CGC
EndIf
...
IF nTipo == 4 .OR. nTipo == 5
	cQuery+=" AND A2_COD||A2_LOJA = SRL.RL_NIFEX "
ELSE
...
ElseIf nTipo == 4 .OR. nTipo == 5
	cQuery+=" AND A2_TIPO = 'X'"
Else
	cQuery+=" AND A2_TIPO = 'J'"
EndIf	

cQuery+=" AND ( SR4.R4_CPFCGC <> ' ' OR A2_TIPO = 'X' )"
cQuery+=" and SR4.R4_CODRET = '"+cCodRet+"'"

cQuery+=" AND SR4.D_E_L_E_T_ <> '*'"
cQuery+=" GROUP BY R4_MAT,A2_TIPO,A2_NOME,A2_COD,A2_LOJA,A2_CGC,R4_ANO,R4_TIPOREN,R4_CODRET,A2_CALCIRF"
cQuery+=" UNION ALL" 
Do Case
	Case nTipo == 1
...
cQuery+=" SELECT 'SE2' R4_MAT, A2_TIPO, A2_NOME, A2_COD, A2_LOJA," 
...
cQuery+=" SUBSTR("+cE2field+",1,4) R4_ANO, '1 Rendimento Tributแvel' TP_REND, '"+cCodRet+"' R4_CODRET, " 
...
ElseIf nTipo == 4
	cQuery+=" AND SE2.E2_CODRET IN ('0422') "
	cQuery+=" AND SED.ED_CALCIRF = 'S' " 
ElseIf nTipo == 5
	cQuery+=" AND SE2.E2_CODRET IN ('8045') "
	cQuery+=" AND SED.ED_CALCIRF = 'S' " 	
Else
	cQuery+=" AND SE2.E2_CODRET  NOT IN ('0422','8045') "
	cQuery+=" AND SED.ED_CALCIRF = 'S' " 
...
/*
Cleuto - 29/12/2020
titulo 0422/8045 nใo precisam estar na query para titulos sem TX pois 0422/8045 sempre deve ser enviado independente se tem TX ou nใo, e isso
jแ foi tratado na query assima
*/
If nTipo == 3 
	cQuery+=" UNION ALL" 
	If nTipo == 3 
...
cQuery+=", SUBSTR(PAI."+cE2field+",1,4) R4_ANO, '5 Imposto retido na fonte' TP_REND, '"+cCodRet+"' R4_CODRET, " 
