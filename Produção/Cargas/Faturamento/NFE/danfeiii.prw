...
#DEFINE MAXITEMC  080 //012 - ALTERADO PARA GEN - DANILO AZEVEDO     // Máxima de caracteres por linha de produtos/serviços
#DEFINE MAXMENLIN 160                                                // Máximo de caracteres por linha de dados adicionais
...
#DEFINE IMPAUTDIR 001                                                // Diretório onde será salvo PDF em caso de automação de impressão
#DEFINE IMPAUTPDF 002                                                // Nome do arquivo PDF em caso de automação de impressão
#DEFINE IMPAUTSER 003                                                // NÚMERO Série NF em caso de automação de impressão
#DEFINE IMPAUTNNF 004                                                // NÚMERO   NF     em caso de automação de impressão
...
User Function DANFE_P1(cIdEnt,cVal1,cVal2,oDanfe,oSetup,_aImprAut,lAutoWMS)
...
/*Inicio customização GEO em 15/02/2018*/
Default _aImprAut := {}
Default lAutoWMS	:= .F.
...
/*Final customização GEO  em 15/02/2018*/
...
If lAutoWMS
	lEnd	:= .F.
	RptStatus({|lEnd| DanfeProc(@oDanfe,@lEnd,cIdEnt,,,@lExistNfe, _aImprAut)},"Imprimindo Danfe...")//DanfeProc(@oDanfe,@lEnd,cIdEnt,,,@lExistNfe, _aImprAut)
Else
	RptStatus({|lEnd| DanfeProc(@oDanfe,@lEnd,cIdEnt,,,@lExistNfe, _aImprAut)},"Imprimindo Danfe...")
EndIf
...
Static Function DanfeProc(oDanfe,lEnd,cIdEnt,cVal1,cVal2,lExistNfe,_aImpPDF)
...
if empty(_aImpPDF)
	...
	_lCont := Pergunte("NFSIGW",.T.) .And. ( !Empty( MV_PAR06 ) .and. MV_PAR06 == 2 .Or. ( Empty( MV_PAR06 ) ))
...
Else
	...
	// Preenche os dados para execução autmática.
	MV_PAR01 := _aImpPDF[IMPAUTNNF]
	MV_PAR02 := _aImpPDF[IMPAUTNNF]
	MV_PAR03 := _aImpPDF[IMPAUTSER]
	MV_PAR04 := 2
	MV_PAR05 := 2
	MV_PAR06 := '2'
	MV_PAR07 := ctod('01/01/94')
	MV_PAR08 := ctod('31/12/49')
...
//PRIVATE oFont07    := TFontEx():New(oDanfe,"Times New Roman",07,07,.F.,.T.,.F.)// 3 //ALTERADO POR DANILO AZEVEDO - GEN
...
Local nTamanho      := 0
Local nFolha        := 1
Local nFolhas       := 1 //0 - ALTERADO PARA O GEN - DANILO AZEVEDO
Local nItem         := 0
Local nMensagem     := 0
Local nBaseICM      := 0
Local nValICM       := 0
...
Local nLinSum       := 0
Local nE            := 0
Local nMaxCod       := 13 //10 - ALTERADO PARA O GEN - DANILO AZEVEDO
...
For nZ := 1 To nLenDet
	...
	nPerDesc	:= round((nDesc*100)/nVTotal,2) //ADICIONADO POR DANILO AZEVEDO - GEN - PERC. DESCONTO
	...
	AADD(aAux, AllTrim(TransForm(nPerDesc,TM(nPerDesc,5,2)))) //ADICIONADO POR DANILO AZEVEDO - GEN - PERC. DESCONTO
	...
	If ( oDet[nX]:_Prod:_uTrib:TEXT <> oDet[nX]:_Prod:_uCom:TEXT )
		...
		lPontilhado := .F. //IIf( nLenDet > 1, .T., lPontilhado ) - ALTERADO PARA O GEN - DANILO AZEVEDO
	...
	While !Empty(cAux) .Or. !Empty(cAuxItem) .Or. !Empty(aAux[1]) .Or. !Empty(aAux[2]) .Or. !Empty(aAux[3]) .Or. !Empty(aAux[4]);
		...
		lPontilhado := .F. //.T.  - ALTERADO PARA O GEN - DANILO AZEVEDO
...
//oDanfe:Say(nLine+265,nBaseTxt+1,"DESTINATARIO /",oFont08N:oFont, , CLR_WHITE, 270 ) //ALTERADO POR DANILO AZEVEDO - GEN
//oDanfe:Say(nLine+260,nBaseTxt+11,"REMETENTE"     ,oFont08N:oFont, ,CLR_WHITE , 270 )  //ALTERADO POR DANILO AZEVEDO - GEN
oDanfe:Say(nLine+265,nBaseTxt+1,"DESTINATARIO /",oFont08N:oFont, , , 270 )
oDanfe:Say(nLine+260,nBaseTxt+11,"REMETENTE"     ,oFont08N:oFont, , , 270 )
...
//oDanfe:Say(nLine+240,MAXBOXH-30,Iif( Empty(aDest[4]),"",ConvDate(aDest[4]) ),oFont08:oFont) //ALTERADO POR DANILO AZEVEDO - GEN
oDanfe:Box(nLine+247,MAXBOXH-40,nLine+272,MAXBOXH+70)
oDanfe:Say(nLine+255,MAXBOXH-30,"HORA ENTRADA/SAÍDA",oFont08N:oFont)
//oDanfe:Say(nLine+265,MAXBOXH-30,aHrEnt[01],oFont08:oFont) //ALTERADO POR DANILO AZEVEDO - GEN
...
//oDanfe:Say(nLine+305,nBaseTxt+7,"FATURA",oFont08N:oFont, ,CLR_WHITE , 270 ) //ALTERADO POR DANILO AZEVEDO - GEN
oDanfe:Say(nLine+305,nBaseTxt+7,"FATURA",oFont08N:oFont, , , 270 )
nBaseTxt += 30
...
//oDanfe:Say(nLine+372,nBaseTxt,"CALCULO",oFont08N:oFont, ,CLR_WHITE , 270 ) //ALTERADO POR DANILO AZEVEDO - GEN
//oDanfe:Say(nLine+360,nBaseTxt+7,"DO",oFont08N:oFont, , CLR_WHITE, 270 ) //ALTERADO POR DANILO AZEVEDO - GEN
//oDanfe:Say(nLine+370,nBaseTxt+14,"IMPOSTO",oFont08N:oFont, , CLR_WHITE, 270 ) //ALTERADO POR DANILO AZEVEDO - GEN
oDanfe:Say(nLine+372,nBaseTxt,"CALCULO",oFont08N:oFont, , , 270 )
oDanfe:Say(nLine+360,nBaseTxt+7,"DO",oFont08N:oFont, , , 270 )
oDanfe:Say(nLine+370,nBaseTxt+14,"IMPOSTO",oFont08N:oFont, , , 270 )
nBaseTxt += 30
...
//oDanfe:Say(nLine+446,nBaseTxt   ,"TRANSPORTADOR/" ,oFont08N:oFont, , CLR_WHITE, 270 ) //ALTERADO POR DANILO AZEVEDO - GEN
//oDanfe:Say(nLine+438,nBaseTxt+7 ,"VOLUMES"        ,oFont08N:oFont, , CLR_WHITE, 270 ) //ALTERADO POR DANILO AZEVEDO - GEN
//oDanfe:Say(nLine+448,nBaseTxt+14,"TRANSPORTADOS"  ,oFont08N:oFont, , CLR_WHITE, 270 ) //ALTERADO POR DANILO AZEVEDO - GEN
oDanfe:Say(nLine+446,nBaseTxt   ,"TRANSPORTADOR/" ,oFont08N:oFont, , , 270 )
oDanfe:Say(nLine+438,nBaseTxt+7 ,"VOLUMES"        ,oFont08N:oFont, , , 270 )
oDanfe:Say(nLine+448,nBaseTxt+14,"TRANSPORTADOS"  ,oFont08N:oFont, , , 270 )
nBaseTxt += 30
...
//oDanfe:Say(nLine+596,nBaseTxt+7,"ISSQN",oFont08N:oFont, , CLR_WHITE, 270 ) //ALTERADO POR DANILO AZEVEDO - GEN
oDanfe:Say(nLine+596,nBaseTxt+7,"ISSQN",oFont08N:oFont, , , 270 )
nBaseTxt += 30
...
// Popula o array de cabeçalho das colunas de produtos/serviços. //ALTERADO POR DANILO AZEVEDO - GEN
/*
aAuxCabec := {;
"COD. PROD",;
"DESCR PROD",;
...
aTamCol := RetTamCol(aAuxCabec, aAux, oDanfe, oFont07:oFont, oFont07:oFont)
...
For nY := 1 To nLenItens
	...
	Else
		...
		//ALTERADO POR DANILO AZEVEDO - GEN
		For nJ := 7 to len(aAux[1])
			nAuxH2 := len(aAux[1][nJ][nY]) + (aColProd[nJ][1] + ((aColProd[nJ][2] - aColProd[nJ][1]) - RetTamTex(aAux[1][nJ][nY], oFont07:oFont, oDanfe)))
			nAuxH2 -= 2
			oDanfe:Say(nLinha, nAuxH2, aAux[1][nJ][nY], oFont07:oFont)
		Next nJ
...
oDanfe:Say(000, INIBOXH+74, Replicate("- ",175), oFont08N:oFont, , , 90 )
...
If lMv_Logod
	...
	cLogoD := Alltrim(GetMv("GEN_FAT059")) + "DANFE" + cDescLogo + ".BMP"
	If !File(cLogoD)
		//ALTERADO PARA O GEN
		//cLogoD	:= GetSrvProfString("Startpath","") + "DANFE" + cEmpAnt + ".BMP"
		cLogoD	:= Alltrim(GetMv("GEN_FAT059")) + "DANFE" + cEmpAnt + ".BMP"
		If !File(cLogoD)
			lMv_Logod := .F.
		EndIf
	EndIf
...
If nFolha == 1
	...
	//oDanfe:Say(nLine+568,nBaseTxt+7,"DADOS DO PRODUTO / SERVIÇO",oFont08N:oFont, ,CLR_WHITE , 270 ) //ALTERADO POR DANILO AZEVEDO -
	oDanfe:Say(nLine+568,nBaseTxt+7,"DADOS DO PRODUTO / SERVIÇO",oFont08N:oFont, , , 270 )
	nBaseTxt += 30
	aColProd := {}
	nAux     := nBaseCol + 30
...
//	AADD(aColProd, {nAux, nAux + aTamCol[10]}) // "PER DESC"
 //	nAux += aTamCol[10]
	...
	oDanfe:Say(nLine+462, nAux + 2,"DESCRIÇÃO", oFont08N:oFont) //ALTERADO PARA O GEN
	nAux += aTamCol[2]
	...
	oDanfe:Say(nLine+462, nAux + 2,"QTD.", oFont08N:oFont) //ALTERADO PARA O GEN
	nAux += aTamCol[7]
	oDanfe:Box(nLine+454, nAux, nLine+469, nAux + aTamCol[8])
	oDanfe:Say(nLine+462, nAux + 2,"V.UNIT.", oFont08N:oFont) //ALTERADO PARA O GEN
	nAux += aTamCol[8]
	oDanfe:Box(nLine+454, nAux, nLine+469, nAux + aTamCol[9])
	oDanfe:Say(nLine+462, nAux + 2,"V.TOTAL", oFont08N:oFont) //ALTERADO PARA O GEN
	nAux += aTamCol[9]
	nAux2 := nAux
	oDanfe:Box(nLine+454, nAux, nLine+469, nAux + aTamCol[10])
	oDanfe:Say(nLine+468, nAux + 2,"R$", oFont08N:oFont) //ALTERADO PARA O GEN
	nAux += aTamCol[10]
	oDanfe:Box(nLine+454, nAux, nLine+469, nAux + aTamCol[11]) //ADICIONADO POR DANILO AZEVEDO - GEN - PERC. DESCONTO
	oDanfe:Say(nLine+468, nAux + 2,"%", oFont08N:oFont) //ADICIONADO POR DANILO AZEVEDO - GEN - PERC. DESCONTO
	oDanfe:Box(nLine+454, nAux2, nLine+461, nAux2 + aTamCol[10] + aTamCol[11]) //ALTERADO PARA O GEN
	oDanfe:Say(nLine+460, nAux2 + 2,"DESCONTO", oFont08N:oFont)
	nAux += aTamCol[11] //ADICIONADO POR DANILO AZEVEDO - GEN - PERC. DESCONTO
	oDanfe:Box(nLine+454, nAux, nLine+469, nAux + aTamCol[12]) //ALTERADO PARA O GEN
	oDanfe:Say(nLine+462, nAux + 2,"V.UNI LIQ", oFont08N:oFont)
	nAux += aTamCol[12] //ALTERADO PARA O GEN
	oDanfe:Box(nLine+454, nAux, nLine+469, nAux + aTamCol[13]) //ALTERADO PARA O GEN
	oDanfe:Say(nLine+462, nAux + 2,"TOTAL LIQ", oFont08N:oFont)
	nAux += aTamCol[13] //ALTERADO PARA O GEN
	oDanfe:Box(nLine+454, nAux, nLine+469, nAux + aTamCol[14]) //ALTERADO PARA O GEN
	oDanfe:Say(nLine+462, nAux + 2,"BC.ICMS", oFont08N:oFont) //ALTERADO PARA O GEN
	nAux += aTamCol[14] //ALTERADO PARA O GEN
	oDanfe:Box(nLine+454, nAux, nLine+469, nAux + aTamCol[15]) //ALTERADO PARA O GEN
	oDanfe:Say(nLine+462, nAux + 2,"V.ICMS", oFont08N:oFont) //ALTERADO PARA O GEN
	nAux += aTamCol[15] //ALTERADO PARA O GEN

	oDanfe:Box(nLine+454, nAux, nLine+469, nAux + aTamCol[16])
	oDanfe:Say(nLine+462, nAux + 2,"V.ICMS ST", oFont08N:oFont)
	nAux += aTamCol[16]

	oDanfe:Box(nLine+454, nAux, nLine+469, nAux + aTamCol[17]) //ALTERADO PARA O GEN
	oDanfe:Say(nLine+462, nAux + 2,"V.IPI", oFont08N:oFont) //ALTERADO PARA O GEN
	nAux  += aTamCol[17] //ALTERADO PARA O GEN
	nAux2 := nAux
	oDanfe:Box(nLine+454, nAux, nLine+469, nAux + aTamCol[18]) //ALTERADO PARA O GEN
	oDanfe:Say(nLine+468, nAux + 2,"ICMS", oFont08N:oFont) //ALTERADO PARA O GEN
	nAux += aTamCol[18] //ALTERADO PARA O GEN
	oDanfe:Box(nLine+454, nAux, nLine+469, nAux + aTamCol[19]) //ALTERADO PARA O GEN
	oDanfe:Say(nLine+468, nAux + 2,"IPI", oFont08N:oFont)
	oDanfe:Box(nLine+454, nAux2, nLine+461, nAux2 + aTamCol[18] + aTamCol[19]) //ALTERADO PARA O GEN
...
Else
	...
	If nPosMsg > 0
		...
		//oDanfe:Say(360,nBaseTxt+7,"DADOS DO PRODUTO / SERVIÇO",oFont08N:oFont, , CLR_WHITE, 270 ) //ALTERADO POR DANILO AZEVEDO - GEN
		oDanfe:Say(360,nBaseTxt+7,"DADOS DO PRODUTO / SERVIÇO",oFont08N:oFont, ,, 270 )
		...
		oDanfe:Say(nLine+462, nAux + 2,"V.UNI LIQ", oFont08N:oFont) //ALTERADO PARA O GEN
		nAux += aTamCol[12]
		oDanfe:Box(nLine+454, nAux, nLine+469, nAux + aTamCol[13])
		oDanfe:Say(nLine+462, nAux + 2,"TOTAL LIQ", oFont08N:oFont) //ALTERADO PARA O GEN
		nAux += aTamCol[13]
		...
		oDanfe:Say(nLine+468, nAux + 2,"IPI", oFont08N:oFont)//ALTERADO PARA O GEN
	...
	Else
		...
		//oDanfe:Say(nLine+768,nBaseTxt+7,"DADOS DO PRODUTO / SERVIÇO",oFont08N:oFont, , CLR_WHITE, 270 ) //ALTERADO POR DANILO AZEVEDO - GEN
		oDanfe:Say(nLine+768,nBaseTxt+7,"DADOS DO PRODUTO / SERVIÇO",oFont08N:oFont, , , 270 )
		nBaseTxt += 30
		aColProd := {}
		nAux     := nBaseCol + 30
		AADD(aColProd, {nAux, nAux + aTamCol[1]}) //"COD. PROD"
		...
		AADD(aColProd, {nAux, nAux + aTamCol[9]}) // "VLR TOTAL"
		nAux += aTamCol[9]
		AADD(aColProd, {nAux, nAux + aTamCol[10]}) // "PER DESC" //ADICIONADO POR DANILO AZEVEDO - GEN - PERC. DESCONTO
		nAux += aTamCol[10] //ADICIONADO POR DANILO AZEVEDO - GEN - PERC. DESCONTO
...
If nFolha ==1
	...
	//oDanfe:Say(MAXBOXV-25,nBaseTxt+1,"DADOS",oFont08N:oFont, , CLR_WHITE, 270 ) //ALTERADO POR DANILO AZEVEDO - GEN
	//oDanfe:Say(MAXBOXV-13,nBaseTxt+11,"ADICIONAIS"    ,oFont08N:oFont, ,CLR_WHITE , 270 ) //ALTERADO POR DANILO AZEVEDO - GEN
	oDanfe:Say(MAXBOXV-25,nBaseTxt+1,"DADOS",oFont08N:oFont, , , 270 )
	oDanfe:Say(MAXBOXV-13,nBaseTxt+11,"ADICIONAIS"    ,oFont08N:oFont, , , 270 )
	nBaseTxt += 30
...
ElseIf nFolha == 2
	...
	//oDanfe:Say(nPosAdicionais,nBaseTxt+1,"DADOS",oFont08N:oFont, ,CLR_WHITE,270)  //ALTERADO POR DANILO AZEVEDO - GEN
	//oDanfe:Say(nPosAdicionais+12,nBaseTxt+11,"ADICIONAIS"    ,oFont08N:oFont, ,CLR_WHITE , 270 ) //ALTERADO POR DANILO AZEVEDO - GEN
	oDanfe:Say(nPosAdicionais,nBaseTxt+1,"DADOS",oFont08N:oFont, , ,270)
	oDanfe:Say(nPosAdicionais+12,nBaseTxt+11,"ADICIONAIS"    ,oFont08N:oFont, , , 270 )
	nBaseTxt += 30
