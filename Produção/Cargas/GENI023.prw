...
±±º          ³Integração Pedido de Vendas com stage                       º±±
±±º          ³Fatura serviço Site GEN                                     º±±
±±º          ³Fatura serviço Curso Forum                                  º±±
...
Local cTesFoPF		:= GetMv("GEN_FAT194",.f.,"")
Local cTesFoPJ		:= GetMv("GEN_FAT205",.F.,"")
Local cTesForum		:= GetMv("GEN_FAT272",.F.,"568")
Local cTes   		:= ""
Local cDtValid     	:= ""
...
While (_cArqTmp)->(!EOF())
	...
	Do While cChavCli == alltrim(str((_cArqTmp)->C5_XPEDOLD))+" "+alltrim(str((_cArqTmp)->DIGITAL))
		...
		If Empty(_cMsg) .AND. (_cArqTmp)->ORIGEM <> "FORUM"
			cMsgDupl	:= ""
			lBlqPed	:= .F.
		...
		If Empty(_cMsg)
			...
			While (_cArqTmp)->(!EOF()) .And. _cPedOld == AllTrim(Str((_cArqTmp)->C5_XPEDOLD)) .and. _cCont <= "99";
				...
				IF (_cArqTmp)->ORIGEM == "FORUM"
					...
					aAdd ( _aCabPd , { "C5_MENNOTA"	, 'REFERENTE A COMPRA DE ['+AllTrim(_cDescPd)+']. PEDIDO ['+alltrim(str((_cArqTmp)->C5_XPEDWEB))+'].' 										, NIL} )
				ELSE	
				...
				IF (_cArqTmp)->ORIGEM == "FORUM" 
					// QUANDO DIFERENTE DE FORUM O PREÇO DE VENDA SERÁ CARREGADO DA TABELA DE PREÇO
...
//				_cCont		:= soma1(_cCont)
				...
				*/
				...
				IF Empty(_cMsg)	
					GENI023B(_aClient,_aCabPd,_aItmPd,_cCodOld,_cPedOld,_cCGC,_cTipo,_cPedWeb,_cNome,_aParcItem,cOrigem,nIdPgt)
				EndIf	
				_cCod := ""
		...
		Else
			MemoWrite ( _cLogPd + SM0->M0_CODIGO + AllTrim(SM0->M0_CODFIL) + "_WEB_" + AllTrim(_cPedWeb) + "_OLD_" + AllTrim(_cPedOld) +"_"+Iif((_cArqTmp)->DIGITAL=1,"DIG_","IMP_")+ ".txt" , _cMsg )
			(_cArqTmp)->(DbSkip())
		EndIf
		_cMsg := Space(0)
	Enddo
...
Static Function GENI023B(_aClient,_aCabPd,_aItmPd,_cCodOld,_cPedOld,_cCGC,_cTipo,_cPedWeb,_cNome,_aParcItem,cOrigem,nIdPgt)
