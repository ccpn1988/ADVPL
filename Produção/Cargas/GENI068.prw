...
±±ºPrograma  ³GENI068   ºAutor  ³Cleuto Lima         º Data ³  09/10/20   º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºDesc.     ³Rotina copia da GENI068 para execução pontual do faturamentoº±±
±±º          ³de diferimentos residuais                                   º±±
...
Conout("GENI068 - Inicio da importação de pedido de vendas de servicos")
...
IF !LockByName("GENI068",.T.,.T.,.T.)
	Conout("GENI068 - Não foi possível executar a rotina neste momento, a mesma já está sendo executada. "+DtoC(DDataBase)+" "+Time())
	Return .F.
EndIf		
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±ÉÍÍÍÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍ»±±
±±ºPrograma  ³GENI068A  ºAutor  ³Angelo Henrique     º Data ³  11/07/14   º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
...
Local _cCont 		:= "01"
Local _cNome		:= ""
Local cTes   		:= GetMv("GEN_FAT194",.F.,"565") // utilizada mesma TES de faturamento do curso forum antigo na 7001 que não gera financeiro
Local cDtValid     	:= ""
...
While (_cArqTmp)->(!EOF())
	...
	IF nQtdPdProc > nLimProc
		Conout("GENI068 - maximo de pedidos por execução atingido - "+cValToChar(nLimProc)+" - encerrando processamento "+DtoC(DDataBase)+" "+Time())
		Exit
	ENDIF
	...
	Do While cChavCli == alltrim(str((_cArqTmp)->C5_XPEDOLD))+" "+alltrim(str((_cArqTmp)->DIGITAL))
		...
		If Empty(_cMsg)
			...
			If SUBSTR(_cCGC,1,8) <> '00000000' .AND. !Empty(_cCGC)
				//Conout("GENI068 - Entrou na validação de CNPJ preenchido com ZERO")
				...
				If (_cArqAli)->(!EOF())
					//Conout("GENI068 - Entrou na validação de adição de loja")
					DbSelectArea("SA1")
					DbSetOrder(3)
			...
			_aClient := {}
			If !Empty(_cCod)
				//Conout("GENI068 - Colocando o código do cliente")
			...
			While (_cArqTmp)->(!EOF()) .And. _cPedOld == AllTrim(Str((_cArqTmp)->C5_XPEDOLD)) .and. _cCont <= "99";
				...
				If nPosAux == 0
					If (_cArqTmp)->TPPUBLI = 28 //Tipo de Publicacao E-Aluguel [Bruno Parreira,22/07/2019]
						cDtValid := "31/12/2019"
						aAdd ( _aCabPd , { "C5_XMSGNFS" , "Nro Pedido: "+alltrim((_cArqTmp)->C5_XPEDWEB)+" - "+AllTrim(_cDescPd)+" - DISPONIBILIZAÇÃO, SEM CESSÃO DEFINITIVA, DE TEXTO E IMAGEM DE LIVROS REF. À COLETÂNEA POR MEIO DA INTERNET - VÁLIDO ATÉ "+cDtValid, NIL} )
					Else 
				...
				Else
					If (_cArqTmp)->TPPUBLI = 28 //Tipo de Publicacao E-Aluguel [Bruno Parreira,22/07/2019]
						cDtValid := "31/12/2019"
						_aCabPd[nPosAux][2] := "Nro Pedido: "+alltrim((_cArqTmp)->C5_XPEDWEB)+" - "+AllTrim(_cDescPd)+" - DISPONIBILIZAÇÃO, SEM CESSÃO DEFINITIVA, DE TEXTO E IMAGEM DE LIVROS REF. À COLETÂNEA POR MEIO DA INTERNET - VÁLIDO ATÉ "+cDtValid
					Else 
				...
				Cleuto - 23/09/2020 - comente este codigo pois a tabela GENESB.PEDIDO não é mais utilizada a muito tempo.
				...
				*/
				...
				// cleuto - 09/11/2020 até o momento a definição é que não teremos financeiro
				// If "VENDA" $ _cTipo
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±ÉÍÍÍÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍ»±±
±±ºPrograma  ³GENI068B  ºAutor  ³Angelo Henrique     º Data ³  14/07/14   º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
...
Private _cLogPd	:= "\logsiga\ped diferimento\"//SUPERGETMV("MV_XCAMLOG",.T.,"\logsiga\ped venda\") //Parâmetro que contém o caminho onde será gravado o arquivo de log de inconsistências
		...
		If !lMsErroAuto .and. lGeraNF
			...
			If _nTotItem == 1
				...
				*'-----------------------------------------------------------------------------------------------------'*
				...
				---------------------------------------------------------*/
			Else
				cMsg := "GENI068 - IMPORTAÇÃO SERVIÇO WEB" + cEnt
				cMsg += cEnt
