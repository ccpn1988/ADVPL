...
ฑฑบDesc.     ณProcessa fila de cancelamento de nota fiscal WMS.           บฑฑ
ฑฑบ          ณ                                                            บฑฑ
...
While !LockByName("JFWMS003",.T.,.T.,.T.)
	...
	Sleep(10)
	If nX > 2     
		Conout("JFWMS003 - Nใo foi possํvel executar a fila WMS neste momento pois a fun็ใo JFWMS003 jแ esta sendo executada por outra processamento!"+DTOC(DDataBase)+" "+Time())
		Return .F.
    EndIf
EndDo
    ...
    IF ZZ5->(!EOF())
    	If SOFTLOCK("ZZ5") 
    		...
    		Else 
	    		...
	    		ZZ5->ZZ5_QTDPRC	:= ZZ5->ZZ5_QTDPRC+1
	    		ZZ5->(MsUnLock())
    			Conout("JFWMS003 - Pedido nใo localizado na base "+ZZ5->ZZ5_IDFILA+" "+Time())
    		EndIF
    	       	
    	Else
    		Conout("JFWMS003 - Nใo foi possํvel obter acesso exclusivo ao registro "+ZZ5->ZZ5_IDFILA+" "+Time())		
    	EndIF
    Else
...
ElseIf SF2->(DbSeek( SC5->C5_FILIAL+SC5->C5_NOTA+SC5->C5_SERIE ))
	...
	Else
		cMotivo	:= "Exclusใo nใo pode ser executada!"
		lRet	:= .F.	
	EndIf
EndIf
	...
	While !(_cAliSC9)->(Eof())
		...
		IF 	SC9->(Recno()) == (_cAliSC9)->SC9RECNO
			...
			If !Empty(SC9->C9_BLCRED) .AND. SC9->C9_BLCRED <> "09" .AND. SC9->C9_BLCRED <> "10"
				...
				a450Grava(1,.T.,.F.,.F.)
				//a450Grava(1,.T.,.T.,.F.) apenas libera็ใo de credito
			EndIF
