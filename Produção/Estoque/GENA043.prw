...
ฑฑบDesc.     ณRotina customizada para automatizar o movimento interno     บฑฑ
ฑฑบ          ณusando arquivo texto                                        บฑฑ
...
Local aSays		:= {}
Local cCadastro	:= "Movimento interno Automแtico"
Local aRet 		:= {}
Local aButtons	:= {}
Local cTabFre	:= ""
...
Private nOpcA	:= 0
	...
	AADD(aSays,OemToAnsi("Esta rotina tem o objetivo realizar movimento interno          "				))
	...
	AADD(aButtons, { 5,.T.,{|o|	ParamBox({;
												...
												{01,"Armazem"				,CriaVar("D2_LOCAL",.F.)	,PesqPict("SD3","D2_LOCAL"),".T.",""/*F3*/,"",20,.T.},;
												{01,"Cod. Mov."				,CriaVar("D3_TM",.F.)		,PesqPict("SD3","D3_TM"),".T.","SF5","",20,.T.},;
...
Static Function LeExcel(cFile,cArmOri,cTm,cSepara,cObserv,lPermParc) 
...
Local aLog		:= {}
Local lContinua	:= .T.
Local cMovOk	:= SuperGetMv("GEN_EST006",.F.,"")
Local nAux		:= 0
...
If SF5->(DbSeek(xFilial("SF5")+cTM))
	If !(cTM $ cMovOk)
		MsgStop("Tipo de Movimento invalido.")
		Return
	Endif
Else
	MsgStop("Tipo de Movimento nใo localizado.")
	Return
Endif
...
For nAux := 1 To Len(aDados)
    ...
    aSld := {}
    aadd(aSld,{"D3_TM"		,cTm			,})
	aadd(aSld,{"D3_COD"		,SB1->B1_COD	,})
	aadd(aSld,{"D3_UM" 		,SB1->B1_UM		,})
	aadd(aSld,{"D3_LOCAL"	,cArmOri		,})
	aadd(aSld,{"D3_QUANT"	,aDados[nAux][LAY_QTD],})
	aadd(aSld,{"D3_EMISSAO"	,dDataBase		,})
  
	DbSelectArea("SB2")
	SB2->(dbSetOrder(1))
	If !SB2->(dbSeek(xFilial("SB2")+cProduto+cLocOri)) //.And. MsgYesNo("Nใo existe registro para este produto no almoxarifado de destino! Deseja criar ?")
		RecLock("SB2",.T.)
		SB2->B2_FILIAL := xFilial("SB2")
		SB2->B2_COD    := cProduto
		SB2->B2_LOCAL  := cLocOri
		MsUnLock()
	EndIf	 
	...
	If lMsErroAuto
		...
		MostraErro(nil,FunName()+"_"+cProduto+".log")
		Aadd(aLog,"Linha:"+StrZero(nAux,4)+" ,Obra "+SB1->B1_COD+" ("+AllTrim(SB1->B1_ISBN)+" - "+AllTrim(SB1->B1_DESC)+") Erro ao gerar documento transferencia : "+cProduto+", arquivo de log "+FunName()+"_"+cProduto+".log")
	Else
		nProc++
	EndIf
