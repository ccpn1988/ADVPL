...
±±ºPrograma  ³GENA071   ºAutor  ³Danilo Azevedo      º Data ³  15/06/15   º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºDesc.     ³Rotina de Aparas que força a devolução quando a nota de     º±±
±±º          ³consignação tem saldo no Gen mas não tem na origem          º±±
...
LockByName("GENA071",.T.,.T.,.T.)
U_GENA071A("03")
UnLockByName("GENA071",.T.,.T.,.T.)
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±ÉÍÍÍÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍ»±±
±±ºPrograma  ³GENA071A  ºAutor  ³Angelo Henrique     º Data ³  29/08/14   º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
...
Local _cTsNoPd3	:= GetMv("GEN_FAT174") //Contém o TES utilizado na nota de entrada das empresas Origem que não controla poder de 3, consignação (positivo) - Vendas
Local _cLogSld	:= ""
Local aItOri	:= {}
Local _cNomArq	:= "" 
Local lNoOri	:= .F. 
		...
		While (_cAliQry)->(!EOF())
			...
			While AllTrim((_cAliQry)->B1_PROC) == _cForn .And. AllTrim((_cAliQry)->B1_LOJPROC) == _cLojFn .and. _nItMax < 100
				...
				If _nPrcVn > 0
					...
					//Substituido bloco acima pelo bloco abaixo para tratar quando nota de saida origem tem 2 itens com mesmo produto e mesma quantidade. [Bruno Parreira, 17/11/2020]
					_cQuery += " (SELECT SUM(B6_SALDO) 
					_cQuery += " FROM "+RetSqlName("SB6")+" B6_ORI 
					_cQuery += " JOIN "+RetSqlName("SD2")+" SD2_ORI 
					_cQuery += " ON SD2_ORI.D2_FILIAL = B6_ORI.B6_FILIAL 
					_cQuery += " AND SD2_ORI.D2_DOC = B6_ORI.B6_DOC 
					_cQuery += " AND SD2_ORI.D2_COD = B6_ORI.B6_PRODUTO 
					_cQuery += " AND SD2_ORI.D2_IDENTB6 = B6_ORI.B6_IDENT 
					_cQuery += " AND SD2_ORI.D_E_L_E_T_ = ' ' 
					_cQuery += " WHERE B6_ORI.B6_FILIAL = '"+AllTrim(_cEmpFl)+"' 
					_cQuery += " AND B6_ORI.B6_DOC = SB6.B6_DOC 
					_cQuery += " AND B6_ORI.B6_SERIE = SB6.B6_SERIE 
					_cQuery += " AND B6_ORI.B6_PRODUTO = SB6.B6_PRODUTO 
					_cQuery += " AND B6_ORI.B6_QUANT = SB6.B6_QUANT 
					_cQuery += " AND B6_ORI.D_E_L_E_T_= ' '
					_cQuery += " AND (CASE WHEN ( 
					_cQuery += " SELECT COUNT(*) FROM "+RetSqlName("SD2")+" SD2_QTD_ORI 
					_cQuery += " WHERE SD2_QTD_ORI.D2_FILIAL = B6_ORI.B6_FILIAL 
					_cQuery += " AND SD2_QTD_ORI.D2_DOC = B6_ORI.B6_DOC 
					_cQuery += " AND SD2_QTD_ORI.D2_COD = B6_ORI.B6_PRODUTO 
					_cQuery += " AND SD2_QTD_ORI.D2_QUANT = B6_ORI.B6_QUANT 
					_cQuery += " ) > 1 THEN ( CASE WHEN SD2_ORI.D2_ITEM = SD1.D1_ITEM THEN 1 ELSE 0 END) ELSE 1 END = 1)) SALDO_ORI 
...
//					_cQuery += " AND SD1.D1_QUANT - SD1.D1_QTDEDEV >= SB6.B6_SALDO
					...
					/*
					// cleuto - 08/11/2017 - removido pois será preciso forçar a devolução das notas que tem saldo no gen
 ...
 /*Saldo a ser gerado na Origem: */	" | "+AllTrim(Str(  IIF( (_cAliQry5)->SALDO_ORI-(_cAliQry)->B2_QATU < 0 , ((_cAliQry5)->SALDO_ORI-(_cAliQry)->B2_QATU)*(-1), (_cAliQry5)->SALDO_ORI-(_cAliQry)->B2_QATU )))
					...
					While (_cAliQry5)->(!EOF())
						...
						BeginSql Alias "TMP_ORI"
								...
								/*AND B6_ORI.B6_SALDO	> 0 para forçar a utilização da nota com saldo no gen */
								AND B6_ORI.%NotDel%
						...
						If _nQtdB6 = 0
							...
							aAdd(_aSldB6,{_nQtdSld, (_cAliQry5)->B6_DOC, (_cAliQry5)->B6_SERIE, cItemAux				, (_cAliQry5)->B6_IDENT, _nPruNit, cIdentB6,lNoSldOri, (_cAliQry5)->D1_ITEM })
							Exit
						EndIf
						...
						If _nQtdB6 > 0
							...
							aAdd(_aSldB6,{_nSldB6Comp, (_cAliQry5)->B6_DOC, (_cAliQry5)->B6_SERIE, cItemAux				, (_cAliQry5)->B6_IDENT, _nPruNit, cIdentB6,lNoSldOri, (_cAliQry5)->D1_ITEM })
					...
					*/
					...
					For _ni := 1 To Len(_aSldB6)
						...
						aAdd ( _alinha 	, 	{ "C6_ITEMORI"	, PADR(AllTrim(_aSldB6[_ni][9]),TAMSX3("C6_ITEMORI")[1])	, Nil})
			...
			If Len(_aItmPv) > 0
				...
				If !_lVcDesc
					...
					If !Empty(_cNtDvCon)
						...
						conout(Time()+ " " + "GENA071 - Inicio do RPC para logar na empresa origem Pedido/Nota de Saída")
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±ÉÍÍÍÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍ»±±
±±ºPrograma  ³GENA071B  ºAutor  ³Angelo Henrique     º Data ³  17/09/14   º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
...
conout(Time()+ " " + "GENA071B - Rotina para execução do Execauto de Geração do Pedido de Vendas e de Geração do Documento de Saída, empresa Matriz (GEN)")
...
If !lMsErroAuto
	...
	conout(Time()+ " " + "GENA071B - Gerou com sucesso o pedido, irá ver se existe a necessidade de desbloquear por crédito na empresa Matriz (GEN)")
	...
	If DbSeek(xFilial("SC9") + SC5->C5_NUM)
		...
		conout(Time()+ " " + "GENA071B - Ira varrer a SC9 para realizar o desbloqueio por crédito na empresa Matriz (GEN)")
		...
		While ( (_cAliSC9)->(!Eof()) .And. (_cAliSC9)->C9_FILIAL == xFilial("SC9") .And. (_cAliSC9)->C9_PEDIDO == SC5->C5_NUM .And. Eval(bValid) )
			If !( (Empty(SC9->C9_BLCRED) .And. Empty(SC9->C9_BLEST)) .OR. (SC9->C9_BLCRED=="10" .And. SC9->C9_BLEST=="10") .OR.;
				SC9->C9_BLCRED=="09" )
				conout(Time()+ " " + "GENA071B - Liberação de Crédito do Pedido de Vendas na empresa Matriz (GEN)")
	...
	If DbSeek(xFilial("SC9")+SC5->C5_NUM)
		...
		conout(Time()+ " " + "GENA071B - Inicio da Geração do Documento de Saída na empresa Matriz (GEN).")
		...
		*'---------------------------------------------------------'*
		...
		If Empty(AllTrim(_cNotaImp))
			conout(Time()+ " " + "GENA071B - Geração do Documento de Saída na empresa Matriz (GEN) apresentou erro .")
			//_cPedExc := SC9->C9_PEDIDO
			...
			conout(Time()+ " " + "GENA071B - Irá alterar o pedido de vendas para poder realizar a exclusão na empresa Matriz (GEN)")
			
			lMsErroAuto := .F.
			MSExecAuto({|x,y,z| Mata410(x,y,z)},_aCabPv,_aItmPv,4)
			
			If !lMsErroAuto
				conout(Time()+ " " + "GENA071B - Alterou o pedido de vendas com sucesso, irá realizar a exclusão na empresa Matriz (GEN).")
				...
				Else
					conout(Time()+ " " + "GENA071B - Não conseguiu excluir o pedido de vendas na empresa Matriz (GEN).")
				...
				EndIf
			Else
				conout(Time()+ " " + "GENA071B - Não conseguiu alterar o pedido de vendas na empresa Matriz (GEN).")
	...
	EndIf
Else
	conout(Time()+ " " + "GENA071B - Não conseguiu gerar o Pedido de Vendas na empresa Matriz (GEN). ")
	_aErro := GetAutoGRLog()
	For _ni := 1 To Len(_aErro)
		_cErroLg += _aErro[_ni] + cEnt
		conout(_cErroLg)
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±ÉÍÍÍÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍ»±±
±±ºPrograma  ³GENA071C  ºAutor  ³Angelo Henrique     º Data ³  17/09/14   º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
...
conout(Time()+ " " + "GENA071C - Rotina de Geração da Nota Fiscal de Entrada (Devolução da Consignação) e Geração da Nota Fiscal de Saída da Venda na empresa Origem ")
conout(Time()+ " " + "GENA071C - Primeiro a Geração da Nota Fiscal de Entrada (Devolução da Consignação) da empresa origem")
...
If lMsErroAuto
	...
	_lRet := .F.
	conout(Time()+ " " + "GENA071C - Não conseguiu gerar a Nota Fiscal de Entrada (Devolução da Consignação) na empresa origem. ")
	_aErro := GetAutoGRLog()
	_cErroLg	:= ""
	For _ni := 1 To Len(_aErro)
		_cErroLg += _aErro[_ni] + cEnt
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±ÉÍÍÍÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍ»±±
±±ºPrograma  ³GENA071   ºAutor  ³Microsiga           º Data ³  08/12/16   º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºDesc.     ³                                                            º±±
±±º          ³                                                            º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
