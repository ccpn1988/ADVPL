	...
	Do Case
		...
		Case UPPER(ALLTRIM(SCV->CV_XOPERA))+'#'$'STONE#'
			...
			Else
				cRet	:= 'STONE'
			EndIf
		Case UPPER(ALLTRIM(SCV->CV_XOPERA))+'#'$'GETNET#'
			If nTipo == 1
				cRet	:= '11020109'
			Else
				cRet	:= 'GETNET'
			EndIf
		Case UPPER(ALLTRIM(SCV->CV_XOPERA))+'#'$'RECORRENCIA#'
			If nTipo == 1
				cRet	:= '11020103'
			Else
				cRet	:= 'RECORRENCIA'
			EndIf			
		Case cFilAnt == '1001' .AND. SA1->A1_EST <> "EX" .AND. SF2->F2_TIPO == "N" .AND. SD2->D2_TES == "568" .AND. Alltrim(SM0->M0_CGC) <> Alltrim(SA1->A1_CGC)
			If nTipo == 1
				cRet	:= IIf( !Empty(SA1->A1_CONTA) , SA1->A1_CONTA ,"11020101")
			Else
				cRet	:= 'GETNET'				
			EndIf			
	EndCase
